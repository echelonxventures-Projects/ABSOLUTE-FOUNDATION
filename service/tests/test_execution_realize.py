"""EC3-B11-U08 — Realization + determinism tests (VC-4, AC-1…7, VC-1…5, evidence, CLI)."""
...
from service.execution_realize import (
    CANONICAL_TYPE_TAG,
    UNIT_VERSION,
    RealizationResult,
...
def _rr(execution):
    trace = build_execution_traceability(
        execution, unit="EC3-B11-U08", forward=(execution.execution_id,)
    )
...
def test_canonical_execution_shape():
    e = build_canonical_execution()
    assert e.type_tag == CANONICAL_TYPE_TAG
    assert e.kind is ExecutionKind.SYNCHRONOUS
    assert e.meta_class == "SMC-08"
    assert e.operation_ref and e.data_refs and e.policy_ref
...
def test_bundle_shape_is_deterministic_and_complete():
    ...
    assert bundle["unit"] == "EC3-B11-U08"
    assert bundle["meta_class"] == "SMC-08"
...
def test_cli_main_returns_zero(tmp_path, capsys):
    ...
    assert rc == 0
    assert "[PASS] EC3-B11-U08 → COMPLETE" in capsys.readouterr().out
...
def test_cli_main_reports_failure_when_not_complete(tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr("service.execution_realize.emit_evidence", _failing_emit)
    rc = main(["--evidence-dir", str(tmp_path)])
    assert rc == 1
    assert "[FAIL] EC3-B11-U08 → NOT COMPLETE" in capsys.readouterr().out


def test_determination_not_complete_for_rejected_execution():
    result = _rr(make_execution("t", "ENG-005:SOE-08:t", kind=ExecutionKind.SYNCHRONOUS))
...
def test_determination_complete_with_conditions_when_compliance_degraded():
    result = _rr(build_canonical_execution())
