"""EC3-B11-U11 — Meta-model validation tests (SMI-01…07 checks + acceptance + negatives)."""
...
from service.model_meta import META_CLASSES, META_RELATIONSHIPS, ONTOLOGY_ENTITIES
from service.model_traceability import build_model_traceability
from service.model_validation import (
...
from service.tests.test_model import a_model


def _trace(model):
    return build_model_traceability(model, unit="EC3-B11-U11", forward=("f",))
...
def test_valid_metamodel_passes_full_suite():
    ...
    # every CCE-shared id the SMC-01 gate suite requires is present and passing
    for shared in (
        "traceability-rooted",
        "meta-class-single",
...
def test_meta_class_closure_check():
    ...
    assert MetaClassClosureCheck().evaluate(
        replace(_subject(), meta_classes=("SMC-01",))
    ).passed is False
    # correct set but does not declare closure
    assert MetaClassClosureCheck().evaluate(
        replace(_subject(), declares_closure=False)
    ).passed is False
...
def test_meta_relationship_closure_check():
    ...
    # outside SMR-01…13
    assert MetaRelationshipClosureCheck().evaluate(
        replace(_subject(), meta_relationships=(*META_RELATIONSHIPS, "SMR-99"))
    ).passed is False
    # not exact (missing one, no outsider)
    assert MetaRelationshipClosureCheck().evaluate(
        replace(_subject(), meta_relationships=tuple(META_RELATIONSHIPS[:-1]))
    ).passed is False
    # correct set but does not declare closure
    assert MetaRelationshipClosureCheck().evaluate(
        replace(_subject(), declares_relationship_closure=False)
    ).passed is False


def test_totality_check():
    assert MetaModelTotalityCheck().evaluate(_subject()).passed is True
    assert MetaModelTotalityCheck().evaluate(
        replace(_subject(), modelled_entities=("SOE-01",))
    ).passed is False
    assert MetaModelTotalityCheck().evaluate(
        replace(_subject(), modelled_relationships=("SOR-01",))
    ).passed is False
    # entities + relationships total, but is_total flag false
    assert MetaModelTotalityCheck().evaluate(
        replace(_subject(), is_total=False)
    ).passed is False


def test_founding_acyclic_check():
    assert FoundingAcyclicCheck().evaluate(_subject()).passed is True
    assert FoundingAcyclicCheck().evaluate(
        replace(_subject(), founding_acyclic=False)
    ).passed is False


def test_members_certified_check():
    assert ModelMembersCertifiedCheck().evaluate(_subject()).passed is True
    assert ModelMembersCertifiedCheck().evaluate(
        replace(_subject(), members_certified=False)
    ).passed is False


def test_map_resolves_check():
    assert ModelMapResolvesCheck().evaluate(_subject()).passed is True
    assert ModelMapResolvesCheck().evaluate(
        replace(_subject(), map_resolves=False)
    ).passed is False


def test_meta_constraints_check():
    assert MetaConstraintsCheck().evaluate(_subject()).passed is True
    assert MetaConstraintsCheck().evaluate(
        replace(_subject(), confers_authority=True)
    ).passed is False


def test_valid_state_check():
    assert ModelValidStateCheck().evaluate(_subject()).passed is True
    assert ModelValidStateCheck().evaluate(
        replace(_subject(), model_state="BOGUS")
    ).passed is False
...
def test_reuse_integrity_check():
    ...
    assert FoundationReuseIntegrityCheck().evaluate(
        replace(_subject(), redefines_el1=True)
    ).passed is False
    assert FoundationReuseIntegrityCheck().evaluate(
        replace(_subject(), substrate_refs=())
    ).passed is False
    assert FoundationReuseIntegrityCheck().evaluate(
        replace(_subject(), members_certified=False)
    ).passed is False
    assert FoundationReuseIntegrityCheck().evaluate(
        replace(_subject(), reuses_by_reference=False)
    ).passed is False


def test_independence_check():
    assert ModelIndependenceCheck().evaluate(_subject()).passed is True
    assert ModelIndependenceCheck().evaluate(
        replace(_subject(), names_technology=True)
    ).passed is False


def test_non_projection_check():
    assert NonProjectionCheck().evaluate(_subject()).passed is True
    assert NonProjectionCheck().evaluate(
        replace(_subject(), non_projection=False)
    ).passed is False


def test_non_constitutive_check():
    assert NonConstitutiveCheck().evaluate(_subject()).passed is True
    assert NonConstitutiveCheck().evaluate(
        replace(_subject(), confers_authority=True)
    ).passed is False
    assert NonConstitutiveCheck().evaluate(
        replace(_subject(), embeds_secret=True)
    ).passed is False
    assert NonConstitutiveCheck().evaluate(
        replace(_subject(), selects_technology=True)
    ).passed is False
    # not authority/secret/tech, but the aggregate non_constitutive flag is false
    assert NonConstitutiveCheck().evaluate(
        replace(_subject(), non_constitutive=False)
    ).passed is False


def test_provisional_disclosure_check():
    assert ProvisionalDisclosureCheck().evaluate(_subject()).passed is True
    assert ProvisionalDisclosureCheck().evaluate(
        replace(_subject(), disclosure={})
    ).passed is False
...
def test_traceability_rooted_check():
    ...
    assert TraceabilityRootedCheck().evaluate(
        replace(_subject(), provenance_chain=())
    ).passed is False
    assert TraceabilityRootedCheck().evaluate(
        replace(_subject(), provenance_chain=("WRONG", "11-SERVICE@x"))
    ).passed is False
    assert TraceabilityRootedCheck().evaluate(
        replace(_subject(), provenance_chain=(_subject().meta_class, "SERVICE-005"))
    ).passed is False
