"""EC3-B11-U04 — Realization + determinism tests (VC-4, AC-1…7, VC-1…5, evidence, CLI)."""
...
from service.interface_realize import (
    CANONICAL_TYPE_TAG,
    UNIT_VERSION,
    RealizationResult,
...
def test_canonical_interface_shape():
    i = build_canonical_interface()
    assert i.type_tag == CANONICAL_TYPE_TAG
    assert i.kind is InterfaceKind.REQUEST_RESPONSE
    assert i.meta_class == "SMC-04"
    assert i.service_ref and i.operations and i.io_refs and i.endpoint_ref
...
def test_meta_validity_and_usl_conformance_complete():
    ...
    for law in ("USL-01", "USL-02", "USL-03", "USL-07", "USL-11", "USL-13", "USL-14", "USL-15"):
        assert usl[law], law
...
def test_bundle_shape_is_deterministic_and_complete():
    ...
    assert bundle["unit"] == "EC3-B11-U04"
    assert bundle["meta_class"] == "SMC-04"
...
def test_cli_main_returns_zero(tmp_path, capsys):
    ...
    assert rc == 0
    assert "[PASS] EC3-B11-U04 → COMPLETE" in capsys.readouterr().out
...
def test_cli_main_reports_failure_when_not_complete(tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr("service.interface_realize.emit_evidence", _failing_emit)
    rc = main(["--evidence-dir", str(tmp_path)])
    assert rc == 1
    assert "[FAIL] EC3-B11-U04 → NOT COMPLETE" in capsys.readouterr().out


def test_determination_not_complete_for_rejected_interface():
    result = _rr(make_interface(
        "t", "ENG-005:SOE-01:svc",
        operations=("ENG-005:SOE-05:kafka.topic",),
        io_refs=("ENG-005:DF-2:req",),
...
def test_determination_complete_with_conditions_when_compliance_degraded():
    result = _rr(build_canonical_interface())
