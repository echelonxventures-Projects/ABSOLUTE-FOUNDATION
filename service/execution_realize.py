"""EC3-B11-U08 — Execution realization orchestrator + evidence emitter.

Performs the realization act for **SMC-08 Execution** under MEP-02 (Band 11 Service
realization): constructs the canonical Universal Execution exemplar, validates it through the
CERTIFIED EC-1 engine (meta-validity V1…V5 + Service-law + SEX principles), certifies it
through the reused CCE ten gates (CC-1…CC-10) + Service compliance (C1…C7), closes its
No-Orphan traceability, and emits a **deterministic, content-addressed evidence bundle**. A
double-realization self-check proves byte-identical determinism (VC-4).

Run as a module::

    python -m service.execution_realize --evidence-dir service/_evidence/EC3-B11-U08
...
from service.execution import Execution, make_execution
from service.execution_certification import certify_execution
from service.execution_meta import EXECUTION_META_CLASS, ExecutionKind
from service.execution_traceability import (
    ExecutionTraceabilityRecord,
    build_execution_traceability,
)
...
#: The realization unit this module realizes (EC-3 Band 11, Unit 08).
REALIZATION_UNIT = "EC3-B11-U08"

#: The realized artifact version (version-pinned certification; USL-12).
UNIT_VERSION = "1.0.0"

#: The canonical Universal Execution exemplar realized to prove the construct — a
#: Synchronous execution carrying out the CERTIFIED-by-reference canonical operation,
#: behaving-as the RUNTIME execution concern (RUNTIME-006) by reference, reading/writing one
#: DF-2 data reference, governed-by the declarative policy by reference.
CANONICAL_TYPE_TAG = "ucos.service.execution.foundation"
CANONICAL_OPERATION_REF = "ENG-005:SOE-05:ucos.service.operation.foundation"
CANONICAL_DATA_REFS = ("ENG-005:DF-2:ucos.data.entity.executed",)
CANONICAL_POLICY_REF = "ENG-005:SOE-09:ucos.service.policy.foundation"
...
_USL_CHECKS: dict[str, str] = {
    ...
    "USL-03": "execution-typed",
    "USL-04": "execution-identified-objectbound",
    "USL-05": "execution-identified-objectbound",
    "USL-06": "execution-contract-fulfilment",
    "USL-10": "execution-runtime-reuse",
    "USL-11": "execution-data-by-reference",
    "USL-12": "lifecycle-valid",
    "USL-15": "technology-independence",
}


def build_canonical_execution() -> Execution:
    """Construct the canonical Synchronous-Execution exemplar (SMC-08)."""
    return make_execution(
        CANONICAL_TYPE_TAG,
        CANONICAL_OPERATION_REF,
        kind=ExecutionKind.SYNCHRONOUS,
        data_refs=CANONICAL_DATA_REFS,
        policy_ref=CANONICAL_POLICY_REF,
...
class RealizationResult:
    """The full, deterministic outcome of the EC3-B11-U08 realization act."""
    ...
    def usl_conformance(self) -> dict[str, bool]:
        ...
        result["USL-01"] = True
        result["USL-13"] = not self.execution.confers_authority()
        result["USL-14"] = not self.execution.confers_authority()
    ...
    def acceptance_criteria(self) -> dict[str, bool]:
        ...
        return {
            ...
            "AC-3": (
                passed.get("execution-typed", False)
                and passed.get("execution-identified-objectbound", False)
            ),
    ...
    def to_bundle(self, *, byte_identical: bool) -> dict[str, Any]:
        return {
            "artifact": "EC3-B11-U08-REALIZATION-EVIDENCE",
...
def realize() -> RealizationResult:
    ...
    execution = build_canonical_execution()
    trace = build_execution_traceability(
        execution,
        unit=REALIZATION_UNIT,
        forward=(
            execution.execution_id,
            "EC3-B11-U08-VALIDATION-EVIDENCE",
            "EC3-B11-U08-CCE-CERTIFICATION",
            "EC3-B11-U08-COMPLETION-REPORT",
        ),
    )
    validation = validate_execution(execution, trace)
    certification = certify_execution(validation, version=UNIT_VERSION)
...
def main(argv: list[str] | None = None) -> int:
    """CLI entry: realize EC3-B11-U08, emit evidence, print the determination."""
    parser = argparse.ArgumentParser(
        prog="ec3-b11-u08-realize",
        description="Realize the Universal Execution (SMC-08) and emit evidence.",
    )
    ...
    print(f"[{'PASS' if complete else 'FAIL'}] EC3-B11-U08 → {summary['determination']}")
