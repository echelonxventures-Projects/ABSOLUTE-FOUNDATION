"""EC3-B11-U01 — Service certification (CCE ten gates CC-1…CC-10 + Service C1…C7).

Certification is *aggregation, not re-judgment* (TP-01 soundness): every gate below
reads only the outcome of the service-layer validation (:mod:`service.service_validation`)
and never re-inspects the service. It runs through the **CERTIFIED EC-1 Certification
Engine** (:class:`engine.certification.engine.CertificationEngine`), issues an
immutable, content-addressed record, and appends it to the append-only, hash-chained
EC-1 :class:`~engine.certification.ledger.CertificationLedger`.

Two evidence families are produced:

* **CC-1…CC-10** — the CCE ten-gate certification (EC-3 AP-3 §8.4), realized as ten
...
* **C1…C7** — the SERVICE-001 §12 Service-compliance conditions, decided on the same
  validation evidence, deterministically and non-coercively (USL-13).
...
class Gate1Architecture(_GateCriterion):
    criterion_id = "CC-1"
    description = "Gate 1 Architecture — no orphan (Service traces the Universe→Code spine)."
...
class Gate3Coverage(_GateCriterion):
    ...
    required_checks = ("service-value-fidelity", "founding-acyclic", "meta-relationships-closed")
...
# ---------------------------------------------------------------------------
# SERVICE-001 §12 — Service compliance (C1…C7)
# ---------------------------------------------------------------------------
...
#: SMC-03/04/05 units; for the atomic Service root they are satisfied by the service's
#: own explicit structure (type + kind + resolvable references).
_COMPLIANCE_CHECKS: dict[str, tuple[str, ...]] = {
    "C1": ("service-typed", "service-identified-objectbound"),
    "C2": ("foundation-reuse-integrity",),
    "C3": ("service-value-fidelity",),
    "C4": ("service-typed", "service-classified", "service-realizes-capability"),
...
_C3_NOTE = "operation I/O (DF-2) scoped to the Operation unit SMC-05; service core value explicit"
_C4_NOTE = "contract/interface/operation scoped to SMC-03/04/05; service structure explicit"


@dataclass(frozen=True, slots=True)
class ServiceComplianceReport:
    """A deterministic SERVICE-001 §12 compliance report (C1…C7)."""
...
def evaluate_service_compliance(validation: ServiceValidation) -> ServiceComplianceReport:
    """Decide the C1…C7 compliance of the validated service on validation evidence."""
...
class ServiceCertification:
    """The bundled outcome of certifying a validated Service."""
...
def certify_service(
    ...
    *,
    ...
    """Run the CCE ten gates + C1…C7 over a validated service and ledger the record.

    Reuses the CERTIFIED EC-1 certification engine and append-only ledger. The result
    is CERTIFIED iff all ten gates close, the ledger chain is intact, and the service is
    Service-COMPLIANT (C1…C7).
    """
    ...
    compliance = evaluate_service_compliance(validation)
