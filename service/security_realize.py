"""EC3-B11-U10 — Security realization orchestrator + evidence emitter.

Performs the realization act for **SMC-10 Security** under MEP-02 (Band 11 Service realization):
constructs the canonical Universal Security exemplar, validates it through the CERTIFIED EC-1
engine (meta-validity V1…V5 + Service-law + SSE principles), certifies it through the reused
...
Run as a module::
    ...
    python -m service.security_realize --evidence-dir service/_evidence/EC3-B11-U10
...
from service.security import Security, make_security
from service.security_certification import certify_security
from service.security_meta import SECURITY_META_CLASS, SecurityKind
from service.security_traceability import (
...
#: The realization unit this module realizes (EC-3 Band 11, Unit 10).
REALIZATION_UNIT = "EC3-B11-U10"
...
#: Confidentiality-Record classifying the CERTIFIED-by-reference canonical Operation +
#: Execution boundary (SMR-09), informed by the canonical declarative Policy (SMR-08
#: governed-by), evaluating via the RUNTIME policy concern (RUNTIME-010) by reference, and
#: reusing DATA-014 data-security classifications for confidentiality over one DF-2 data
#: reference (SMR-13/SSE-06). Confidentiality is a data-bearing SXH-10 kind, so this exemplar
#: materially exercises SSE-06 (DATA-014 reuse), C6 (RUNTIME reuse by reference), and C7 (the
#: evaluative, non-enforcing governing law USL-14).
CANONICAL_TYPE_TAG = "ucos.service.security.foundation"
CANONICAL_SUBJECT_REFS = (
    "ENG-005:SOE-05:ucos.service.operation.foundation",
    "ENG-005:SOE-08:ucos.service.execution.foundation",
)
CANONICAL_POLICY_REFS = ("ENG-005:SOE-09:ucos.service.policy.foundation",)
...
_USL_CHECKS: dict[str, str] = {
    ...
    "USL-03": "security-typed",
    "USL-04": "security-identified-objectbound",
    "USL-05": "security-identified-objectbound",
    "USL-10": "security-runtime-reuse",
    "USL-11": "security-data-by-reference",
    "USL-12": "lifecycle-valid",
    "USL-13": "security-evaluative-nonenforcing",
    "USL-15": "technology-independence",
}


def build_canonical_security() -> Security:
    """Construct the canonical Confidentiality-Security exemplar (SMC-10)."""
    return make_security(
        CANONICAL_TYPE_TAG,
        CANONICAL_SUBJECT_REFS,
        kind=SecurityKind.CONFIDENTIALITY,
        policy_refs=CANONICAL_POLICY_REFS,
...
class RealizationResult:
    """The full, deterministic outcome of the EC3-B11-U10 realization act."""
    ...
    def usl_conformance(self) -> dict[str, bool]:
        ...
        result["USL-01"] = True
        result["USL-14"] = passed.get("security-evaluative-nonenforcing", False)
    ...
    def to_bundle(self, *, byte_identical: bool) -> dict[str, Any]:
        return {
            "artifact": "EC3-B11-U10-REALIZATION-EVIDENCE",
...
def realize() -> RealizationResult:
    ...
    trace = build_security_traceability(
        ...
        forward=(
            ...
            "EC3-B11-U10-VALIDATION-EVIDENCE",
            "EC3-B11-U10-CCE-CERTIFICATION",
            "EC3-B11-U10-COMPLETION-REPORT",
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    """CLI entry: realize EC3-B11-U10, emit evidence, print the determination."""
    parser = argparse.ArgumentParser(
        prog="ec3-b11-u10-realize",
        description="Realize the Universal Security (SMC-10) and emit evidence.",
    )
    ...
    print(f"[{'PASS' if complete else 'FAIL'}] EC3-B11-U10 → {summary['determination']}")
