"""EC3-B11-U01 — Traceability record (No-Orphan closure, GOV-001-T3).

Every realized Service records a closed lineage (EC-3 AP-3 §5/§9):
...
* **Substrate:** the frozen foundations ``ENG-001…005`` (EL-1), ``RL-F2``, ``PL-F2``,
  ``DF-2`` — referenced, not redefined (USL-02).
* **Forward:** the realized Service construct + its validation/certification evidence.
...
from service.service import FOUNDATION_REUSE, Service
...
TRACE_FORMAT = "ucos-service-traceability/1.0.0"

#: The anchor prefix the backward lineage must close to (frozen Service spec set).
_ANCHOR_PREFIX = "11-SERVICE@"


@dataclass(frozen=True, slots=True)
class TraceabilityRecord:
    """A deterministic, closed No-Orphan lineage record for a realized Service."""
    ...
    def closed(self) -> bool:
        """True iff backward closes to the 11-SERVICE anchor and substrate/forward present."""
        return (
            self.rooted
...
def build_traceability(
    service: Service, *, unit: str, forward: tuple[str, ...]
) -> TraceabilityRecord:
    """Assemble the closed No-Orphan lineage record for ``service``."""
