"""EC3-B10-U05 — Realization orchestrator + evidence emitter (Storage Foundation).

Performs the realization act authorized by ``EC3-B10-DATA-REALIZATION-PACKAGE-005``
(derived from the frozen constitutional corpus — DATA-010 — since the package file is
not present): constructs the canonical **Local-Topology** exemplar that **persists the
CERTIFIED DMC-02 Entity by reference** (DMR-05), schema-conformant to the CERTIFIED
DMC-05 Schema (DTA-07 / DTA-K3), binding persistence behavior **by reference** to the
frozen RUNTIME state concern (DMR-11 / DTA-02). This extends the spine to
``Storage persists (schema-conformant) Entity bears Attribute values Datum``
(DMR-05 → DMR-01 → DMR-02, with ``describes`` from DMC-05).

It is **abstract topology only** — it declares placement (locus set), durability, and
distribution as *concepts*, and names **no** engine, database, file/serialization
format, query language, broker, warehouse/lake, cache, cloud data service, or vendor
(UDL-11 Storage Independence — materially enforced).

It validates the topology through the CERTIFIED EC-1 engine (meta-validity V1…V5 +
Data-law UDL incl. **UDL-11**), certifies it through the CCE ten gates (CC-1…CC-10,
reused from DMC-01) + Data compliance (C1…C7, C6 materially exercised), closes its
No-Orphan traceability (incl. ``schema-aligned → DMC-05`` + ``persists → DMC-02``), and
...
Run as a module::
    ...
    python -m data.storage_realize                 # emit evidence + determination
    python -m data.storage_realize --evidence-dir data/_evidence/EC3-B10-U05

This is engineering execution only (DE-05): it asserts no constitutional finality and —
for storage specifically — **no operational/deployment/production readiness**
(DATA-010 §14); it selects no technology, reuses the CERTIFIED EC-1 + DMC-01/02/05
surfaces by reference, and writes nothing outside the caller-provided evidence dir.
"""
...
from data.storage import PersistedEntityRef, Storage, make_storage, runtime_ref_for
from data.storage_certification import StorageCertification, certify_storage
from data.storage_meta import STORAGE_META_CLASS, DurabilityLevel, StorageKind, StorageState
from data.storage_traceability import build_storage_traceability
from data.storage_validation import StorageValidation, validate_storage
from data.traceability import TraceabilityRecord
from engine.certification.contracts import content_hash

#: The realization unit this module realizes (EC-3 Band 10, Unit 05).
REALIZATION_UNIT = "EC3-B10-U05"

#: The realized artifact version (version-pinned certification; URS-L-20).
UNIT_VERSION = "1.0.0"

#: The canonical topology name/type — the topology that persists the CERTIFIED entity.
CANONICAL_STORAGE_NAME = "ucos.data.storage.foundation"
CANONICAL_STORAGE_TYPE_TAG = "ucos.core.storage"

#: The canonical placement locus (abstract; names no product) and RUNTIME binding.
CANONICAL_LOCUS = "locus.primary"
CANONICAL_RUNTIME_CONCEPT = "state.persist"
...
_UDL_CHECKS: dict[str, str] = {
    ...
    "UDL-03": "storage-typed",
    "UDL-04": "storage-identified",
    "UDL-05": "storage-identified",
...
def build_canonical_storage() -> Storage:
    """Construct the canonical Local-Topology exemplar (DMC-06).

    Persists the CERTIFIED DMC-02 canonical Master-Entity by reference (DMR-05),
    schema-conformant to the CERTIFIED DMC-05 canonical Schema (DTA-07), with an
    explicit single-locus placement (DTA-03), declared durable durability (DTA-04), and
    persistence bound by reference to the RUNTIME state concern (DMR-11). Abstract
    topology only — names no storage technology (UDL-11).
    """
    entity = build_canonical_entity()
    schema = build_canonical_entity_schema()
    return make_storage(
        CANONICAL_STORAGE_NAME,
        CANONICAL_STORAGE_TYPE_TAG,
        (CANONICAL_LOCUS,),
        ((entity, schema),),
        runtime_ref_for(CANONICAL_RUNTIME_CONCEPT),
        kind=StorageKind.LOCAL,
        durability=DurabilityLevel.DURABLE,
        state=StorageState.DEFINED,
...
class RealizationResult:
    """The full, deterministic outcome of the EC3-B10-U05 realization act."""

    storage: Storage
    persisted_entity: Entity
    conformance_schema: Schema
    trace: TraceabilityRecord
    validation: StorageValidation
    certification: StorageCertification
    ...
    def udl_conformance(self) -> dict[str, bool]:
        ...
        result["UDL-01"] = True
        result["UDL-13"] = not self.storage.confers_authority()
        result["UDL-14"] = not self.storage.confers_authority()
    ...
    def acceptance_criteria(self) -> dict[str, bool]:
        ...
        return {
            ...
            "AC-3": (  # typed + named + identified + persists a subject by reference
                passed.get("storage-typed", False)
                and passed.get("storage-named", False)
                and passed.get("storage-identified", False)
                and passed.get("storage-persists-entities", False)
            ),
            "AC-4": (  # explicit placement + declared durability + consistent topology + kind
                passed.get("storage-placement-explicit", False)
                and passed.get("storage-durability-declared", False)
                and passed.get("storage-topology-consistent", False)
                and passed.get("storage-classified", False)
            ),
            "AC-5": passed.get("storage-independence", False),  # no technology (material UDL-11)
            "AC-6": passed.get("non-constitutive", False),  # no authority/secret
            "AC-7": True,  # realized under data/**; frozen corpus + DMC-02/05 unmodified (guarded)
            "AC-8": self.trace.closed,  # full traceability recorded (incl. schema-aligned/persists)
        }
    ...
    def spine_closure(self) -> dict[str, bool]:
        """The ``Storage persists (schema-conformant) Entity …`` spine-closure facts."""
        return {
            "persists_entity": self.storage.persists_entity(self.persisted_entity.entity_id),
            "schema_aligned": self.storage.is_schema_aligned(),
            "binds_runtime_by_reference": self.storage.binds_runtime_by_reference(),
            "technology_neutral": not self.storage.names_technology(),
        }
    ...
    def to_bundle(self, *, byte_identical: bool) -> dict[str, Any]:
        ...
        return {
            "artifact": "EC3-B10-U05-REALIZATION-EVIDENCE",
            "unit": REALIZATION_UNIT,
            "meta_class": STORAGE_META_CLASS,
            "version": UNIT_VERSION,
            "authority": "ENGINEERING-EXECUTION-ONLY",
            "storage": self.storage.to_dict(),
            "persists_entity": self.persisted_entity.to_dict(),
            "conformance_schema": self.conformance_schema.to_dict(),
            "spine": (
                "Storage persists (schema-conformant) Entity bears Attribute values Datum "
...
def realize() -> RealizationResult:
    ...
    persisted_entity = build_canonical_entity()
    conformance_schema = build_canonical_entity_schema()
    storage = make_storage(
        CANONICAL_STORAGE_NAME,
        CANONICAL_STORAGE_TYPE_TAG,
        (CANONICAL_LOCUS,),
        ((persisted_entity, conformance_schema),),
        runtime_ref_for(CANONICAL_RUNTIME_CONCEPT),
        kind=StorageKind.LOCAL,
        durability=DurabilityLevel.DURABLE,
        state=StorageState.DEFINED,
    ...
    trace = build_storage_traceability(
        storage,
        unit=REALIZATION_UNIT,
        forward=(
            storage.storage_id,
            "EC3-B10-U05-VALIDATION-EVIDENCE",
            "EC3-B10-U05-CCE-CERTIFICATION",
            "EC3-B10-U05-COMPLETION-REPORT",
        ),
    )
    validation = validate_storage(storage, trace)
    certification = certify_storage(validation, version=UNIT_VERSION)
    return RealizationResult(
        storage=storage,
        persisted_entity=persisted_entity,
...
def emit_evidence(evidence_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "storage_id": result.storage.storage_id,
        "persists_entity_id": result.persisted_entity.entity_id,
        "conformance_schema_id": result.conformance_schema.schema_id,
...
def main(argv: list[str] | None = None) -> int:
    """CLI entry: realize EC3-B10-U05, emit evidence, print the determination."""
    parser = argparse.ArgumentParser(
        prog="ec3-b10-u05-realize",
        description="Realize the Storage Foundation (DMC-06) and emit evidence.",
    )
    ...
    print(f"[{'PASS' if complete else 'FAIL'}] EC3-B10-U05 → {summary['determination']}")
