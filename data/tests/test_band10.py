"""EC3-B10-U12 — Band-10 completion construct tests (record invariants + roll-ups)."""
...
def _completion(**overrides) -> Band10Completion:
    base = make_band10_completion(
...
def test_make_band10_completion_inventories_eleven_units():
    c = _completion()
...
def test_identity_and_digest_are_deterministic():
    ...
    assert a.band_id.startswith("UCOS-BAND10-")
    assert len(a.structure_digest) == 64
    assert a.meta_class == "BAND-10"
...
def test_unit_certification_ref_to_dict():
    ref = UnitCertificationRef("EC3-B10-U01", "DMC-01", "Datum", "DATA-001…005", "cid", True)
    d = ref.to_dict()
...
def test_meta_class_coverage_complete_true_and_false():
    ...
    # drop the Datum unit → coverage no longer exactly DMC-01…10
    dropped = _completion(units=tuple(u for u in c.units if u.meta_class != "DMC-01"))
...
def test_technology_secret_authority_scans():
    ...
    tech = _completion(type_tag="postgres-backed-completion")
...
def test_canonical_core_excludes_band_id():
    ...
    assert core["band_class"] == "BAND-10"
