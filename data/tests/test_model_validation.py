...
def test_meta_class_closure_check():
    ...
    assert MetaClassClosureCheck().evaluate(
        replace(_subject(), meta_classes=("DMC-01",))
    ).passed is False
...
def test_meta_relationship_closure_check():
    ...
    assert MetaRelationshipClosureCheck().evaluate(
        replace(_subject(), meta_relationships=(*META_RELATIONSHIPS, "DMR-99"))
    ).passed is False
...
def test_totality_check():
    ...
    assert MetaModelTotalityCheck().evaluate(
        replace(_subject(), modelled_entities=("DOE-01",))
    ).passed is False
    assert MetaModelTotalityCheck().evaluate(
        replace(_subject(), modelled_relationships=("DOR-01",))
    ).passed is False
...
def test_traceability_rooted_check():
    ...
    assert TraceabilityRootedCheck().evaluate(
        replace(_subject(), provenance_chain=("WRONG", "10-DATA@x"))
    ).passed is False
    assert TraceabilityRootedCheck().evaluate(
        replace(_subject(), provenance_chain=(_subject().meta_class, "DATA-005"))
    ).passed is False
