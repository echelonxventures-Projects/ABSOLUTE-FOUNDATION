...
sample: :func:`verify_invertible` reconstructs all 1220 source records from their objects
...
def test_every_artifact_becomes_exactly_one_object(assimilation_report):
    assert assimilation_report.artifacts_read == 1220
    assert assimilation_report.objects_minted == 1220
...
def test_the_report_serializes_and_summarizes(assimilation_report):
    ...
    assert record["counts"]["artifacts_read"] == 1220
