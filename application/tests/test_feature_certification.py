"""EC3-B12-U04 — Feature certification tests (CCE CC-1…CC-10 + Application C1…C7)."""
...
from application.feature_certification import (
    Gate1Architecture,
    Gate2Dependencies,
    Gate3Coverage,
    Gate4Validation,
...
from application.feature_meta import FeatureKind
from application.feature_traceability import build_traceability
from application.feature_validation import feature_checks, validate_feature
...
def test_certify_feature_certifies_valid_feature():
    cert = certify_feature(_validated(), version="1.0.0")
    assert cert.certified is True
    assert cert.decision.certified is True
    assert cert.ledger.verify() is True
    assert cert.compliance.compliant is True
    assert cert.decision.certification_id.startswith("UCOS-CERT-AMC-04-")


def test_all_ten_gates_pass_for_valid_feature():
    cert = certify_feature(_validated(), version="1.0.0")
...
def test_compliance_report_all_conditions_pass():
    report = evaluate_feature_compliance(_validated())
...
def test_compliance_c3_and_c4_are_materially_exercised():
    report = evaluate_feature_compliance(_validated())
    ...
    assert "materially exercised" in by_id["C4"]["note"]
...
def test_compliance_report_serializes_deterministically():
    ...
    assert payload["target_id"].startswith("UCOS-FEATURE-")


def test_compliance_fails_when_a_backing_check_fails():
    # A technology-bearing feature fails technology-independence → C7 fails.
    ...
    report = evaluate_feature_compliance(validation)
    by_id = {c["id"]: c for c in report.conditions}
    assert by_id["C7"]["status"] == "fail"
    assert report.compliant is False


def test_certification_not_certified_for_rejected_feature():
    techy = make_feature("t", CAP, ("ENG-005:SF-2:kafka.consumer",))
    trace = build_traceability(techy, unit=UNIT, forward=FWD)
    validation = validate_feature(techy, trace)
    cert = certify_feature(validation, version="1.0.0")
...
def test_certification_reuses_supplied_ledger():
    ...
    c1 = certify_feature(_validated(make_feature("a", CAP, OPS)), version="1.0.0", ledger=ledger)
    c2 = certify_feature(_validated(make_feature("b", CAP, OPS)), version="1.0.0", ledger=ledger)
...
def test_gate2_fails_when_operation_not_composed():
    assert Gate2Dependencies().evaluate(
        _good_subject(blocking_failures=("feature-consumes-operation",))
    ).status is CriterionStatus.FAIL
