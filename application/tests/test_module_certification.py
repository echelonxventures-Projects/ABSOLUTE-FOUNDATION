"""EC3-B12-U03 — Module certification tests (CCE CC-1…CC-10 + Application C1…C7)."""
...
from application.module_certification import (
    Gate1Architecture,
    Gate2Dependencies,
    Gate3Coverage,
    Gate4Validation,
...
from application.module_validation import module_checks, validate_module
...
def test_certify_module_certifies_valid_module():
    cert = certify_module(_validated(), version="1.0.0")
    assert cert.certified is True
    assert cert.decision.certified is True
    assert cert.ledger.verify() is True
    assert cert.compliance.compliant is True
    assert cert.decision.certification_id.startswith("UCOS-CERT-AMC-03-")


def test_all_ten_gates_pass_for_valid_module():
    cert = certify_module(_validated(), version="1.0.0")
...
def test_compliance_report_all_conditions_pass():
    report = evaluate_module_compliance(_validated())
...
def test_compliance_c3_c4_c5_carry_scope_notes():
    report = evaluate_module_compliance(_validated())
    by_id = {c["id"]: c for c in report.conditions}
    assert "note" in by_id["C3"]
    assert "AMC-02/04" in by_id["C3"]["note"]
...
def test_compliance_report_serializes_deterministically():
    ...
    assert payload["target_id"].startswith("UCOS-MODULE-")


def test_compliance_fails_when_a_backing_check_fails():
    # A technology-bearing module fails technology-independence → C7 fails.
    techy = make_module("t", ("ENG-005:AMC-04:kafka.consumer",))
    trace = build_traceability(techy, unit=UNIT, forward=FWD)
    validation = validate_module(techy, trace)
    report = evaluate_module_compliance(validation)
    by_id = {c["id"]: c for c in report.conditions}
    assert by_id["C7"]["status"] == "fail"
    assert report.compliant is False


def test_certification_not_certified_for_rejected_module():
    techy = make_module("t", ("ENG-005:AMC-04:kafka.consumer",))
    trace = build_traceability(techy, unit=UNIT, forward=FWD)
    validation = validate_module(techy, trace)
    cert = certify_module(validation, version="1.0.0")
...
def test_certification_reuses_supplied_ledger():
    ...
    c1 = certify_module(_validated(make_module("a", FEATS)), version="1.0.0", ledger=ledger)
    c2 = certify_module(_validated(make_module("b", FEATS)), version="1.0.0", ledger=ledger)
...
def test_gate2_fails_when_composition_not_by_reference():
    assert Gate2Dependencies().evaluate(
        _good_subject(blocking_failures=("composition-by-reference",))
    ).status is CriterionStatus.FAIL
