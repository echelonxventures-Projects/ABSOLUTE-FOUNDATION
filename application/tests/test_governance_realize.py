"""EC3-B12-U10 — Realization orchestrator tests (AC-1…AC-7 + VC-1…VC-5 + determinism + CLI)."""
...
from application.governance_meta import REALIZATION_UNIT, GovernanceKind
from application.governance_realize import (
...
def test_canonical_governance_is_the_foundation_exemplar():
    g = build_canonical_governance()
    assert isinstance(g, Governance)
    assert g.governed_constructs() == CANONICAL_SUBJECT_REFS
    assert g.kind is GovernanceKind.CONFORMANCE
    assert g.facet() == "conformance"
    assert g.is_founding_acyclic() is True
    assert g.evaluative_nonenforcing() is True
    assert g.binds_runtime_policy() is True
    assert g.meta_class == "AMC-10"
...
def test_ual_conformance_applicable_laws_pass():
    ...
    # scoped to Feature/Module/Application/Composition/Interaction/State units — recorded N/A).
    assert set(ual) == {
        "UAL-01", "UAL-02", "UAL-03", "UAL-04", "UAL-05",
        "UAL-10", "UAL-12", "UAL-14", "UAL-15",
    }
    assert ual["UAL-01"] is True  # structural
    assert ual["UAL-10"] is True  # governance-evaluate binds RL-F2 by reference
    assert ual["UAL-14"] is True  # governing — record-only, non-enforcing
...
def test_governance_compliance_all_pass():
    comp = realize().governance_compliance()
...
def test_emit_evidence_writes_all_files(tmp_path):
    ...
    assert summary["governance_id"].startswith("UCOS-GOVERNANCE-")
    assert summary["certification_id"].startswith("UCOS-CERT-AMC-10-")
    expected = {
...
def test_cli_main_returns_zero_on_complete(tmp_path, capsys):
    ...
    assert "EC3-B12-U10 → COMPLETE" in out
    assert "PASS" in out
...
def test_determination_not_complete_for_rejected_result():
    ...
    from application.governance_certification import certify_governance
    from application.governance_realize import RealizationResult
    from application.governance_traceability import build_traceability
    from application.governance_validation import validate_governance
    ...
    techy = make_governance(
        "t",
        ("ENG-005:AMC-01:camunda.workflow", "ENG-005:AMC-04:ucos.demo.feature"),
    )
    trace = build_traceability(techy, unit=REALIZATION_UNIT, forward=())
    validation = validate_governance(techy, trace)
    certification = certify_governance(validation, version="1.0.0")
    result = RealizationResult(
        governance=techy, trace=trace, validation=validation, certification=certification
    )
