"""EC3-B12-U03 — Realization orchestrator tests (AC-1…AC-7 + VC-1…VC-5 + determinism + CLI)."""
...
from application.module_meta import REALIZATION_UNIT
from application.module_realize import (
...
def test_canonical_module_is_the_foundation_exemplar():
    mod = build_canonical_module()
    assert isinstance(mod, Module)
    assert mod.type_tag == CANONICAL_TYPE_TAG
    assert set(mod.feature_refs) == set(CANONICAL_FEATURE_REFS)
    assert mod.meta_class == "AMC-03"
...
def test_ual_conformance_all_pass_and_includes_derived_laws():
    ...
    assert ual["UAL-07"] is True  # module bounded / cohesive (the governing law)
    assert ual["UAL-09"] is True  # composition by reference
...
def test_module_compliance_all_pass():
    comp = realize().module_compliance()
...
def test_emit_evidence_writes_all_files(tmp_path):
    ...
    assert summary["module_id"].startswith("UCOS-MODULE-")
    expected = {
...
def test_cli_main_returns_zero_on_complete(tmp_path, capsys):
    ...
    assert "EC3-B12-U03 → COMPLETE" in out
    assert "PASS" in out
...
def test_determination_not_complete_for_rejected_result():
    ...
    from application.module_certification import certify_module
    from application.module_realize import RealizationResult
    from application.module_traceability import build_traceability
    from application.module_validation import validate_module

    # Technology-bearing module fails validation + certification; empty-forward trace
    ...
    certification = certify_module(validation, version="1.0.0")
    result = RealizationResult(
        module=techy, trace=trace, validation=validation, certification=certification
    )
