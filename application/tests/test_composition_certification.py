"""EC3-B12-U08 — Composition certification tests (CCE CC-1…CC-10 + Application C1…C7)."""
...
def _validated(composition):
    trace = build_traceability(
        composition, unit="EC3-B12-U08", forward=(composition.composition_id, "X")
    )
    return validate_composition(composition, trace)


# -- happy path: full certification over a valid composition ----------------------


def test_certify_valid_composition_is_certified_and_ledgered():
    validation = _validated(make_composition("ucos.demo.composition", MEMBERS,
                                             assembled_ref=ASSEMBLED))
    cert = certify_composition(validation, version="1.0.0")
    assert cert.certified is True
    assert cert.decision.certified is True
    assert cert.ledger.verify() is True
    assert cert.compliance.compliant is True
    assert cert.decision.certification_id.startswith("UCOS-CERT-AMC-08-")
...
def test_gate2_dependencies_pass_and_fail():
    ...
    bad = _ok_subject(blocking_failures={"composition-by-reference"})
...
def test_compliance_all_pass_with_governing_notes():
    validation = _validated(make_composition("t", MEMBERS, assembled_ref=ASSEMBLED))
    report = evaluate_composition_compliance(validation)
    ...
    # C5 is the governing materially-exercised condition (composition by reference, UAL-09)
    assert "note" in conditions["C5"]
    assert "UAL-09" in conditions["C5"]["note"]
    # C3/C4/C6 carry scoping/analog notes
    for cid in ("C3", "C4", "C6"):
...
def test_compliance_report_to_dict_shape():
    validation = _validated(make_composition("t", MEMBERS, assembled_ref=ASSEMBLED))
    d = evaluate_composition_compliance(validation).to_dict()
...
def test_compliance_fails_when_a_backing_check_fails():
    # A technology-bearing composition fails technology-independence → C7 fails.
...
def test_certification_not_certified_for_rejected_validation():
    techy = make_composition("t", ("ENG-005:AMC-03:kubernetes.pod", MEMBERS[1]),
                             assembled_ref=ASSEMBLED)
    validation = _validated(techy)
    cert = certify_composition(validation, version="1.0.0")
