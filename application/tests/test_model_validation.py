"""EC3-B12-U11 — Meta-model validation tests (AMI-01…07 checks + acceptance + negatives)."""
...
from application.model_meta import META_CLASSES, META_RELATIONSHIPS, ONTOLOGY_ENTITIES
from application.model_traceability import build_model_traceability
from application.model_validation import (
...
from application.tests.test_model import a_model


def _trace(model):
    return build_model_traceability(model, unit="EC3-B12-U11", forward=("f",))
...
def test_valid_metamodel_passes_full_suite():
    ...
    # every CCE-shared id the AMC-01 gate suite requires is present and passing
    for shared in (
        "traceability-rooted",
        "meta-class-single",
...
def test_meta_class_closure_check():
    ...
    assert MetaClassClosureCheck().evaluate(
        replace(_subject(), meta_classes=("AMC-01",))
    ).passed is False
...
def test_meta_relationship_closure_check():
    ...
    assert MetaRelationshipClosureCheck().evaluate(
        replace(_subject(), meta_relationships=(*META_RELATIONSHIPS, "AMR-99"))
    ).passed is False
...
def test_totality_check():
    ...
    assert MetaModelTotalityCheck().evaluate(
        replace(_subject(), modelled_entities=("AOE-01",))
    ).passed is False
    assert MetaModelTotalityCheck().evaluate(
        replace(_subject(), modelled_relationships=("AOR-01",))
    ).passed is False
...
def test_traceability_rooted_check():
    ...
    assert TraceabilityRootedCheck().evaluate(
        replace(_subject(), provenance_chain=("WRONG", "12-APPLICATION@x"))
    ).passed is False
    assert TraceabilityRootedCheck().evaluate(
        replace(_subject(), provenance_chain=(_subject().meta_class, "APPLICATION-005"))
    ).passed is False
