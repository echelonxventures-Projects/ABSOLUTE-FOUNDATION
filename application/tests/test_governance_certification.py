"""EC3-B12-U10 — Governance certification tests (CCE CC-1…CC-10 + Application C1…C7)."""
...
_ALL_CHECKS = (
    ...
    "governance-behavior-by-reference",
...
def _validated(governance):
    trace = build_traceability(
        governance, unit="EC3-B12-U10", forward=(governance.governance_id, "X")
    )
    return validate_governance(governance, trace)


# -- happy path: full certification over a valid governance record ----------------


def test_certify_valid_governance_is_certified_and_ledgered():
    validation = _validated(_record())
    cert = certify_governance(validation, version="1.0.0")
    assert cert.certified is True
    assert cert.decision.certified is True
    assert cert.ledger.verify() is True
    assert cert.compliance.compliant is True
    assert cert.decision.certification_id.startswith("UCOS-CERT-AMC-10-")
...
def test_gate2_dependencies_pass_and_fail():
    ...
    bad = _ok_subject(blocking_failures={"governance-behavior-by-reference"})
...
def test_compliance_all_pass_with_governing_notes():
    validation = _validated(_record())
    report = evaluate_governance_compliance(validation)
    ...
    # C7 is the governing materially-exercised condition (record-only non-enforcing, UAL-14/15)
    assert "note" in conditions["C7"]
    assert "UAL-14" in conditions["C7"]["note"]
    # C3/C4/C5/C6 carry scoping/analog notes
...
def test_compliance_report_to_dict_shape():
    validation = _validated(_record())
    d = evaluate_governance_compliance(validation).to_dict()
...
def test_compliance_fails_when_a_backing_check_fails():
    # A technology-bearing record fails technology-independence → C7 fails.
...
def test_certification_not_certified_for_rejected_validation():
    techy = make_governance("t", ("ENG-005:AMC-01:kubernetes.pod", SUBJECTS[1]))
    validation = _validated(techy)
    cert = certify_governance(validation, version="1.0.0")
