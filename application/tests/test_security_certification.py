"""EC3-B12-U09 — Security certification tests (CCE CC-1…CC-10 + Application C1…C7)."""
...
_ALL_CHECKS = (
    ...
    "security-behavior-by-reference",
...
def _validated(security):
    trace = build_traceability(
        security, unit="EC3-B12-U09", forward=(security.security_id, "X")
    )
    return validate_security(security, trace)


# -- happy path: full certification over a valid security record ------------------


def test_certify_valid_security_is_certified_and_ledgered():
    validation = _validated(_record())
    cert = certify_security(validation, version="1.0.0")
    assert cert.certified is True
    assert cert.decision.certified is True
    assert cert.ledger.verify() is True
    assert cert.compliance.compliant is True
    assert cert.decision.certification_id.startswith("UCOS-CERT-AMC-09-")
...
def test_gate2_dependencies_pass_and_fail():
    ...
    bad = _ok_subject(blocking_failures={"security-behavior-by-reference"})
...
def test_compliance_all_pass_with_governing_notes():
    validation = _validated(_record())
    report = evaluate_security_compliance(validation)
    ...
    # C7 is the governing materially-exercised condition (evaluative non-enforcing, UAL-14/15)
...
def test_compliance_report_to_dict_shape():
    validation = _validated(_record())
    d = evaluate_security_compliance(validation).to_dict()
...
def test_certification_not_certified_for_rejected_validation():
    techy = make_security("t", ("ENG-005:AMC-01:kubernetes.pod", SUBJECTS[1]))
    validation = _validated(techy)
    cert = certify_security(validation, version="1.0.0")
