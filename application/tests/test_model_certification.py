"""EC3-B12-U11 — Meta-model certification tests (CCE CC-1…CC-10 + Application C1…C7)."""

from __future__ import annotations

from application.model_certification import (
    certify_model,
    evaluate_model_compliance,
)
from application.model_traceability import build_model_traceability
from application.model_validation import validate_model
from application.tests.test_model import a_model


def _validation(model=None):
    model = model or a_model()
    trace = build_model_traceability(model, unit="EC3-B12-U11", forward=("f",))
...
def test_metamodel_is_certified_all_ten_gates_close():
    ...
    assert cert.decision.certification_id.startswith("UCOS-CERT-UAM-")
...
def test_application_compliance_c1_c7_all_pass_with_c5_material():
...
def test_application_compliance_to_dict_shape():
