"""EC3-B12-U07 — Realization orchestrator tests (AC-1…AC-7 + VC-1…VC-5 + determinism + CLI)."""
...
from application.state_meta import REALIZATION_UNIT, StateKind
from application.state_realize import (
...
def test_canonical_state_is_the_foundation_exemplar():
    st = build_canonical_state()
    assert isinstance(st, State)
    assert st.holder_ref == CANONICAL_HOLDER_REF
    assert st.kind is StateKind.INTERACTION
    assert st.facet() == "interaction"
    assert st.context_is_bound() is True
    assert st.binds_runtime_state() is True
    assert st.presents_data() is True
    assert st.meta_class == "AMC-07"
...
def test_ual_conformance_applicable_laws_pass():
    ...
    # Only the laws that apply to the State are recorded (UAL-06/07/08/09/11 are scoped to
    # Feature/Module/Application/Composition/Interaction units — recorded N/A elsewhere).
    assert set(ual) == {
        "UAL-01", "UAL-02", "UAL-03", "UAL-04", "UAL-05",
        "UAL-10", "UAL-12", "UAL-13", "UAL-14", "UAL-15",
    }
    assert ual["UAL-01"] is True  # structural
    assert ual["UAL-12"] is True  # forward-only recorded lifecycle + context (governing)
    assert ual["UAL-10"] is True  # binds RL-F2 by reference
...
def test_state_compliance_all_pass():
    comp = realize().state_compliance()
...
def test_emit_evidence_writes_all_files(tmp_path):
    ...
    assert summary["state_id"].startswith("UCOS-STATE-")
    expected = {
...
def test_cli_main_returns_zero_on_complete(tmp_path, capsys):
    ...
    assert "EC3-B12-U07 → COMPLETE" in out
    assert "PASS" in out
...
def test_determination_not_complete_for_rejected_result():
    ...
    from application.state_certification import certify_state
    from application.state_realize import RealizationResult
    from application.state_traceability import build_traceability
    from application.state_validation import validate_state

    # Technology-bearing state fails validation + certification; empty-forward trace is not
    ...
    validation = validate_state(techy, trace)
    certification = certify_state(validation, version="1.0.0")
    result = RealizationResult(
        state=techy, trace=trace, validation=validation, certification=certification
    )
