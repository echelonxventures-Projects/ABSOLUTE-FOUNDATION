"""EC3-B12-U11 — Meta-model realization tests (integration + determinism + evidence)."""
...
from application import model_realize
from application.model_meta import META_CLASSES, META_INVARIANTS
from application.model_realize import (
    MemberCertification,
...
def test_realize_members_are_all_ten_and_certified():
    ...
    assert by_class["AMC-01"] == "UCOS-CERT-AMC-01-d998321c1b00d7ff"
    assert by_class["AMC-05"] == "UCOS-CERT-AMC-05-3e5a7af2eb3e485f"
    assert by_class["AMC-10"] == "UCOS-CERT-AMC-10-daa79bb6f252508a"
...
def test_ual_conformance_all_satisfied():
    ual = realize().ual_conformance()
    assert ual["UAL-01"] is True
    assert ual["UAL-02"] is True  # materially exercised (reuse over ten CERTIFIED units)
...
def test_certification_gates_and_application_compliance_rollups():
    result = realize()
    assert result.certification_gates() == {f"CC-{n}": True for n in range(1, 11)}
    assert result.application_compliance() == {f"C{n}": True for n in range(1, 8)}
...
def test_integration_closure_all_true():
    ...
    assert closure["closure_AMI_01"] is True
    assert closure["non_projection_AMI_07"] is True


def test_bundle_shape_and_spine():
    bundle = realize().to_bundle(byte_identical=True)
    assert bundle["unit"] == "EC3-B12-U11"
...
def test_main_returns_zero_on_complete(tmp_path, capsys):
    ...
    assert "EC3-B12-U11 → COMPLETE" in out
...
def test_member_certification_dataclass_roundtrip():
    m = MemberCertification("AMC-02", "EC3-B12-U02", True, "UCOS-CERT-AMC-02-" + "a" * 16)
