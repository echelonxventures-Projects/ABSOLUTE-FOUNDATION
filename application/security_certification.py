"""EC3-B12-U09 — Security certification (CCE ten gates CC-1…CC-10 + Application C1…C7).

Certification is *aggregation, not re-judgment* (TP-01 soundness): every gate below reads only
the outcome of the security-layer validation (:mod:`application.security_validation`) and never
re-inspects the record. It runs through the **CERTIFIED EC-1 Certification Engine**
...
from application.security_meta import APPLICATION_COMPLIANCE
from application.security_validation import SecurityValidation
...
class Gate1Architecture(_GateCriterion):
    criterion_id = "CC-1"
    description = "Gate 1 Architecture — no orphan (Security traces the Universe→Code spine)."
...
class Gate2Dependencies(_GateCriterion):
    ...
    required_checks = ("foundation-reuse-integrity", "security-behavior-by-reference")
...
#: Security record, **C7 (selects no technology, confers no authority, embeds no secret — UAL-15
#: — together with the evaluative, non-enforcing nature — UAL-14) is THE governing
#: materially-exercised condition** of AMC-09: the record classifies protection concerns and
#: enacts nothing. C6 (workflow/state bind RL-F2 — UAL-10/12) is materially exercised through
#: the security-evaluate RL-F2 binding + forward-only lifecycle. C3 (capability delivered via
#: SF-2 + DF-2 data by reference — UAL-06/13) is scoped to the Feature/Capability units
#: (AMC-02/04); the security analog is materially exercised — the record presents DF-2/DATA-014
#: data by reference (AMR-14 / UAL-13). C4 (modules bounded, features explicit, interactions
#: typed — UAL-07/08/11) is scoped to the Module/Feature/Interaction units; the security analog
#: is materially exercised — the record classifies declared application boundaries (AMR-08 /
#: SEC-07). C5 (composition ENG-005 refs + founding acyclic — UAL-09) is scoped to the
#: Composition unit; the security analog is materially exercised — the record binds all links by
#: ENG-005 reference and participates in no founding edge (vacuously acyclic).
_COMPLIANCE_CHECKS: dict[str, tuple[str, ...]] = {
    "C1": ("security-typed", "security-identified-objectbound"),
    "C2": ("foundation-reuse-integrity",),
    "C3": ("security-data-by-reference",),
    "C4": ("security-classifies-boundary", "security-classified"),
    "C5": (
        "security-no-new-connection",
        "founding-acyclic",
        "meta-relationships-closed",
    ),
    "C6": ("security-behavior-by-reference", "lifecycle-valid"),
    "C7": (
        "non-constitutive",
...
_C3_NOTE = (
    "capability delivery via SF-2 operations (UAL-06) is scoped to the Feature/Capability units "
    "(AMC-02/04); a security record classifies protection concerns, consuming no operation — the "
    "data clause (UAL-13) is materially exercised: the record presents DF-2/DATA-014 data by "
    "reference (AMR-14 / SEC-06)"
)
_C4_NOTE = (
    "the module/feature/interaction clauses (UAL-07/08/11) are scoped to the Module (AMC-03), "
    "Feature (AMC-04), and Interaction (AMC-06) units; the security analog is materially "
    "exercised — the record classifies declared application boundaries (AMR-08 / SEC-07)"
)
_C5_NOTE = (
    "the composition clause (UAL-09) is scoped to the Composition unit (AMC-08); the security "
    "analog is materially exercised — the record binds every subject/governance/data/behavior "
    "link by ENG-005 reference and participates in no founding edge (founding graph vacuously "
    "acyclic)"
)
_C6_NOTE = (
    "materially exercised — the security-evaluate behavior binds the frozen RL-F2 RUNTIME policy "
    "concern by reference (§7 / AMK-05) and the record advances forward-only through the "
    "AOS-01…06 lifecycle (UAL-10/12)"
)
_C7_NOTE = (
    "materially exercised — THE governing condition of AMC-09: the record is declarative, "
    "evaluative, and non-enforcing (UAL-14), selects no security/cryptographic technology "
    "(SEC-05), grants no access and issues no credential (SEC-04 / SEC-C2), confers no authority "
    "and embeds no secret (UAL-15 / SEC-09 / RR-07)"
)
...
def evaluate_security_compliance(
    ...
    """Decide the C1…C7 compliance of the validated security record on validation evidence."""
...
class SecurityCertification:
    """The bundled outcome of certifying a validated Security record."""
...
def certify_security(
    ...
    *,
    ...
    """Run the CCE ten gates + C1…C7 over a validated security record and ledger the record.
