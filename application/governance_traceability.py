"""EC3-B12-U10 — Traceability record (No-Orphan closure, GOV-001-T3).

Every realized Governance record records a closed lineage (EC-3 AP-4 §5/§9):

* **Backward:** ``AMC-10 → APPLICATION-014 → APPLICATION-005 → APPLICATION-001 →
  ARCH-APPLICATION-001 → 12-APPLICATION@b7e7657``.
* **Substrate:** the frozen foundations ``ENG-001…005`` (EL-1), ``RL-F2`` —
  referenced, not redefined (UAL-02). The CERTIFIED AMC-01 Application / AMC-04 Feature
  (the governed boundaries, governed-by), the AMC-09 Security (conformance-of, secured-by),
  the AMC-07 State (holds-state, lifecycle-record), and the RL-F2 RUNTIME policy
  (governance-evaluate, §7) are referenced through the record's relationships, never redefined.
* **Forward:** the realized Governance record + its validation/certification evidence.
...
from application.governance import FOUNDATION_REUSE, Governance
...
TRACE_FORMAT = "ucos-application-governance-traceability/1.0.0"
...
class TraceabilityRecord:
    """A deterministic, closed No-Orphan lineage record for a realized Governance record."""
...
def build_traceability(
    governance: Governance, *, unit: str, forward: tuple[str, ...]
) -> TraceabilityRecord:
