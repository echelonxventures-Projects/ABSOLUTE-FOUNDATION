"""EC3-B12-U07 — Traceability record (No-Orphan closure, GOV-001-T3).

Every realized State records a closed lineage (EC-3 AP-4 §5/§9):

* **Backward:** ``AMC-07 → APPLICATION-011 → APPLICATION-005 → APPLICATION-001 →
  ARCH-APPLICATION-001 → 12-APPLICATION@b7e7657``.
* **Substrate:** the frozen foundations ``ENG-001…005`` (EL-1), ``RL-F2``, ``DF-2`` —
  referenced, not redefined (UAL-02). The CERTIFIED AMC-06 Interaction (holds-state), the
  DF-2 data (presents-data), and the RL-F2 state concern (behaves-as) are referenced
  through the state's relationships, never redefined.
* **Forward:** the realized State construct + its validation/certification evidence.
...
from application.state import FOUNDATION_REUSE, State
...
TRACE_FORMAT = "ucos-application-state-traceability/1.0.0"
...
class TraceabilityRecord:
    """A deterministic, closed No-Orphan lineage record for a realized State."""
...
def build_traceability(
    state: State, *, unit: str, forward: tuple[str, ...]
) -> TraceabilityRecord:
    """Assemble the closed No-Orphan lineage record for ``state``."""
