"""EC3-B12-U02 — Capability certification (CCE ten gates CC-1…CC-10 + Application C1…C7).

Certification is *aggregation, not re-judgment* (TP-01 soundness): every gate below reads
only the outcome of the capability-layer validation
(:mod:`application.capability_validation`) and never re-inspects the capability. It runs
...
from application.capability_meta import APPLICATION_COMPLIANCE
from application.capability_validation import CapabilityValidation
...
class Gate1Architecture(_GateCriterion):
    criterion_id = "CC-1"
    description = "Gate 1 Architecture — no orphan (Capability traces the Universe→Code spine)."
...
class Gate2Dependencies(_GateCriterion):
    ...
    required_checks = ("foundation-reuse-integrity", "capability-consumes-operation")
...
#: Capability, C3 (capability delivered via SF-2 operations under contract + DF-2 data by
#: reference) is **materially exercised** — the capability consumes an SF-2 operation
#: (AMR-13) and references DF-2 data (AMR-14). C4 (module cohesion / feature explicitness
#: / interaction typedness) is scoped to the AMC-03/04/06 units; the capability satisfies
#: it through its own explicit, bounded scope (CAP-06 / CAP-C1).
_COMPLIANCE_CHECKS: dict[str, tuple[str, ...]] = {
    ...
    "C4": ("capability-bounded", "capability-typed", "capability-classified"),
...
_C3_NOTE = (
    "materially exercised at the capability level: capability consumes an SF-2 operation "
    "under contract (AMR-13 / CAP-05 / UAL-06) and references DF-2 delivered data "
    "(AMR-14 / CAP-07 / UAL-13), both by reference"
)
_C4_NOTE = (
    "module cohesion/feature explicitness/interaction typedness scoped to AMC-03/04/06; "
    "capability declares an explicit, decidable, bounded scope (CAP-06 / CAP-C1 / UAL-08)"
)
...
def evaluate_capability_compliance(
...
class CapabilityCertification:
    """The bundled outcome of certifying a validated Capability."""
