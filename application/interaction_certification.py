"""EC3-B12-U06 — Interaction certification (CCE ten gates CC-1…CC-10 + Application C1…C7).

Certification is *aggregation, not re-judgment* (TP-01 soundness): every gate below reads
only the outcome of the interaction-layer validation
(:mod:`application.interaction_validation`) and never re-inspects the interaction. It runs
...
from application.interaction_meta import APPLICATION_COMPLIANCE
from application.interaction_validation import InteractionValidation
...
class Gate1Architecture(_GateCriterion):
    criterion_id = "CC-1"
    description = "Gate 1 Architecture — no orphan (Interaction traces the Universe→Code spine)."
...
class Gate2Dependencies(_GateCriterion):
    ...
    required_checks = ("foundation-reuse-integrity", "interaction-presents-data")
...
#: Interaction, **C4 (modules bounded, features explicit, interactions typed — UAL-07/08/11)
#: is materially exercised** — it is THE governing condition for AMC-06: interaction
#: typedness + a decidable direction + an abstract presentation surface (UAL-11). C7 (no
#: technology / authority / secret) is also strongly exercised — an interaction selects no
#: rendering technology, UI framework, or design system (INT-03). C3 (data via DF-2 by
#: reference — UAL-13) is exercised through the presented DF-2 data (AMR-14); the SF-2
#: capability-delivery clause (UAL-06) is scoped to the Feature/Capability units
#: (AMC-02/04). C5 (composition by ENG-005 reference + founding acyclic — UAL-09) is
#: satisfied structurally: the interaction engages its feature by ENG-005 reference through
#: the founding engaged-through edge with no cycle. C6 (RL-F2 by reference + forward-only
#: lifecycle — UAL-10/12) is exercised through the behaves-as / holds-state bindings.
_COMPLIANCE_CHECKS: dict[str, tuple[str, ...]] = {
    "C1": ("interaction-typed", "interaction-identified-objectbound"),
    "C2": ("foundation-reuse-integrity",),
    "C3": ("interaction-presents-data",),
    "C4": (
        "interaction-classified",
        "interaction-direction-decidable",
        "interaction-surface-abstract",
    ),
    "C5": ("founding-acyclic", "meta-relationships-closed", "interaction-engages-feature"),
    "C6": ("behavior-by-reference", "lifecycle-valid", "interaction-holds-state"),
    "C7": ("non-constitutive", "technology-independence"),
}

_C3_NOTE = (
    "the interaction presents its exchanged data as DF-2 data by reference (AMR-14 / "
    "INT-06 / UAL-13); the capability-delivery-via-SF-2 clause (UAL-06) is scoped to the "
    "Feature/Capability units (AMC-02/04) — an interaction engages a feature and presents "
    "its data, it does not itself deliver capability by consuming operations"
)
_C4_NOTE = (
    "materially exercised at the interaction level: the interaction is typed and declares "
    "a decidable direction (input/command/query/response) over an abstract presentation "
    "surface (UAL-11 / INT-01/03/07, THE governing law of AMC-06); module boundedness "
    "(UAL-07) and feature-contract explicitness (UAL-08) are scoped to the Module (AMC-03) "
    "and Feature (AMC-04) units"
)
_C5_NOTE = (
    "the interaction engages its feature by ENG-005 reference through the founding "
    "engaged-through edge (AMR-05, Feature → Interaction) with no self-founding, so its "
    "founding graph is acyclic (AMK-03 / INT-C3); structural module/application composition "
    "(UAL-09) is the Composition concern (AMC-08)"
)
_C6_NOTE = (
    "exercised at the interaction level: the interaction binds the frozen RL-F2 event/state "
    "concern by reference (behaves-as, AMR-11) and holds/advances interaction/session state "
    "forward-only (holds-state, AMR-06) — UAL-10/12; it re-founds no runtime or state concern"
)


@dataclass(frozen=True, slots=True)
class InteractionComplianceReport:
...
def evaluate_interaction_compliance(
    validation: InteractionValidation,
) -> InteractionComplianceReport:
    """Decide the C1…C7 compliance of the validated interaction on validation evidence."""
    ...
    return InteractionComplianceReport(
...
@dataclass(frozen=True, slots=True)
class InteractionCertification:
    """The bundled outcome of certifying a validated Interaction."""

    decision: CertificationDecision
    evidence: CertificationEvidence
    ledger_entry: CertificationLedgerEntry
    ledger: CertificationLedger
    compliance: InteractionComplianceReport
...
def certify_interaction(
    ...
    *,
    ...
    """Run the CCE ten gates + C1…C7 over a validated interaction and ledger the record.

    Reuses the CERTIFIED EC-1 certification engine and append-only ledger. The result is
    CERTIFIED iff all ten gates close, the ledger chain is intact, and the interaction is
    ...
    compliance = evaluate_interaction_compliance(validation)
    return InteractionCertification(
        decision=decision,
