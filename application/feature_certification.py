"""EC3-B12-U04 — Feature certification (CCE ten gates CC-1…CC-10 + Application C1…C7).

Certification is *aggregation, not re-judgment* (TP-01 soundness): every gate below reads
only the outcome of the feature-layer validation (:mod:`application.feature_validation`)
and never re-inspects the feature. It runs through the **CERTIFIED EC-1 Certification
...
from application.feature_meta import APPLICATION_COMPLIANCE
from application.feature_validation import FeatureValidation
...
class Gate1Architecture(_GateCriterion):
    criterion_id = "CC-1"
    description = "Gate 1 Architecture — no orphan (Feature traces the Universe→Code spine)."
...
class Gate2Dependencies(_GateCriterion):
    ...
    required_checks = ("foundation-reuse-integrity", "feature-consumes-operation")
...
#: Feature, **C3 (delivers capability via SF-2 operations + DF-2 data by reference —
#: UAL-06/13) AND C4 (features explicit + interactions typed — UAL-08/11) are BOTH
#: materially exercised** — the Feature is the load-bearing unit of experience-over-
#: operation and is the single construct that materially discharges both conditions at
#: once (unlike U02 where only C3, or U03 where C4/C5).
_COMPLIANCE_CHECKS: dict[str, tuple[str, ...]] = {
    "C1": ("feature-typed", "feature-identified-objectbound"),
    "C2": ("foundation-reuse-integrity",),
    "C3": ("feature-consumes-operation", "feature-presents-data", "feature-delivers-capability"),
    "C4": (
...
_C3_NOTE = (
    "materially exercised at the feature level: the feature delivers capability by "
    "composing one or more SF-2 operations under contract (AMR-13 / FEA-04 — UAL-06, the "
    "governing law) and presents typed I/O as DF-2 data by reference (AMR-14 / FEA-05 — "
    "UAL-13); it re-founds no service and re-models no data"
)
_C4_NOTE = (
    "materially exercised at the feature level: the feature's declaration is complete "
    "(delivered capability, composed operations, typed I/O, and interaction all explicit — "
    "FEA-03 / UAL-08) and it is engaged through a typed interaction (AMR-05 / FEA-06 — "
    "UAL-11, the interaction-typedness law neither U02 nor U03 exercised)"
)
_C5_NOTE = (
    "the feature's founding relations (groups AMR-03, engaged-through AMR-05) form a DAG "
    "and all constituents are bound by ENG-005 reference (UAL-09); module boundedness is "
    "the Module concern (AMC-03)"
)


@dataclass(frozen=True, slots=True)
class FeatureComplianceReport:
...
def evaluate_feature_compliance(
    validation: FeatureValidation,
) -> FeatureComplianceReport:
    """Decide the C1…C7 compliance of the validated feature on validation evidence."""
    ...
    return FeatureComplianceReport(
...
@dataclass(frozen=True, slots=True)
class FeatureCertification:
    """The bundled outcome of certifying a validated Feature."""

    decision: CertificationDecision
    evidence: CertificationEvidence
    ledger_entry: CertificationLedgerEntry
    ledger: CertificationLedger
    compliance: FeatureComplianceReport
...
def certify_feature(
    ...
    *,
    ...
    """Run the CCE ten gates + C1…C7 over a validated feature and ledger the record.

    Reuses the CERTIFIED EC-1 certification engine and append-only ledger. The result is
    CERTIFIED iff all ten gates close, the ledger chain is intact, and the feature is
    ...
    compliance = evaluate_feature_compliance(validation)
    return FeatureCertification(
        decision=decision,
