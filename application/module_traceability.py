"""EC3-B12-U03 — Traceability record (No-Orphan closure, GOV-001-T3).

Every realized Module records a closed lineage (EC-3 AP-4 §5/§9):

* **Backward:** ``AMC-03 → APPLICATION-007 → APPLICATION-005 → APPLICATION-001 →
  ARCH-APPLICATION-001 → 12-APPLICATION@b7e7657``.
* **Substrate:** the frozen foundations ``ENG-001…005`` (EL-1), ``RL-F2``, ``PL-F2`` —
  referenced, not redefined (UAL-02). SF-2/DF-2 + the CERTIFIED AMC-01/AMC-02 units are
  referenced through the module's relationships (composed-into an Application; groups
  Features that deliver Capabilities), never redefined.
* **Forward:** the realized Module construct + its validation/certification evidence.
...
from application.module import FOUNDATION_REUSE, Module
...
TRACE_FORMAT = "ucos-application-module-traceability/1.0.0"
...
class TraceabilityRecord:
    """A deterministic, closed No-Orphan lineage record for a realized Module."""
...
def build_traceability(
    module: Module, *, unit: str, forward: tuple[str, ...]
) -> TraceabilityRecord:
    """Assemble the closed No-Orphan lineage record for ``module``."""
