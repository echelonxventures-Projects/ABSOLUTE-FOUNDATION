"""EC3-B13-U04 — Realization orchestrator + evidence emitter.
...
Charter §3.2/§4.1 STAGE 2, WBS row EC3-B13-U04 — the last remaining Stage-2 Resource):
constructs the canonical Universal Infrastructure Storage-Hosting exemplar, validates it
through the CERTIFIED EC-1 engine (meta-validity WF-1…12 + Infrastructure-law UIL-01…15),
certifies it through the CCE ten gates (CC-1…CC-10) + Infrastructure compliance (C1…C7),
closes its No-Orphan traceability, and emits a **deterministic, content-addressed evidence
bundle**. A double-realization self-check proves byte-identical determinism (VC-4).

Run as a module::

    python -m infrastructure.storage_realize
    python -m infrastructure.storage_realize --evidence-dir infrastructure/_evidence/EC3-B13-U04
...
from infrastructure.storage_certification import StorageCertification, certify_storage
from infrastructure.storage_meta import (
    INFRASTRUCTURE_META_CLASS,
    REALIZATION_UNIT,
    InfrastructureState,
)
from infrastructure.storage_traceability import TraceabilityRecord, build_traceability
from infrastructure.storage_validation import StorageValidation, validate_storage

#: The realized artifact version (version-pinned certification).
UNIT_VERSION = "1.0.0"

#: The canonical Universal Infrastructure Storage-Hosting exemplar realized to prove the
#: construct: a storage-hosting resource with a declared, unbounded capacity, located at
#: an (abstract) Locality by reference, that hosts the frozen DATA-010 datum by reference.
CANONICAL_TYPE_TAG = "ucos.infrastructure.storage.foundation"
CANONICAL_LOCALITY_REF = "ENG-005:INFRASTRUCTURE-011:locality.foundation"
CANONICAL_DATUM_HOST_REF = DEFAULT_DATUM_HOST_REF
CANONICAL_CAPACITY_AMOUNT = 1
CANONICAL_CAPACITY_UNIT = "storage-unit"
...
#: applicable to the Storage-Hosting Resource). WF-4/6/8/9/10 are scoped to other leaf
#: meta-classes. WF-5 (a Resource declares capacity and locality) and WF-7 (hosts a
#: DATA-010 datum by reference) are the governing rules.
_META_VALIDITY_CHECKS: dict[str, str] = {
    "WF-1": "meta-class-single",
    "WF-2": "meta-constraints",
    "WF-3": "founding-acyclic",
    "WF-5": "infra-storage-declares-capacity-locality",
    "WF-7": "infra-storage-hosts-data-by-reference",
...
_UIL_CHECKS: dict[str, str] = {
    ...
    "UIL-03": "infra-storage-typed",
    "UIL-04": "infra-storage-identified-objectbound",
    "UIL-05": "infra-storage-identified-objectbound",
    "UIL-07": "infra-storage-honors-boundaries",
    "UIL-08": "infra-storage-declares-capacity-locality",
    "UIL-11": "infra-storage-hosts-data-by-reference",
    "UIL-13": "scaling-unbounded",
    "UIL-15": "technology-independence",
}


def build_canonical_storage_hosting_resource() -> StorageHostingResource:
    """Construct the canonical Universal Infrastructure Storage-Hosting exemplar."""
    return make_storage_hosting_resource(
        CANONICAL_TYPE_TAG,
        CANONICAL_LOCALITY_REF,
        datum_ref=CANONICAL_DATUM_HOST_REF,
...
class RealizationResult:
    """The full, deterministic outcome of the EC3-B13-U04 realization act."""

    resource: StorageHostingResource
    ...
    def acceptance_criteria(self) -> dict[str, bool]:
        ...
        return {
            ...
            "AC-3": (
                passed.get("infra-storage-typed", False)
                and passed.get("infra-storage-identified-objectbound", False)
            ),
    ...
    def storage_compliance(self) -> dict[str, bool]:
    ...
    def determination(self, *, byte_identical: bool) -> str:
        ...
        if (
            ...
            and all(vc.values())
            and gates_ok
            and all(self.storage_compliance().values())
        ):
            return "COMPLETE"
    ...
    def to_bundle(self, *, byte_identical: bool) -> dict[str, Any]:
        ...
        return {
            "artifact": "EC3-B13-U04-REALIZATION-EVIDENCE",
            ...
            "criteria": {
                ...
                "infrastructure_compliance": self.storage_compliance(),
            },
...
def realize() -> RealizationResult:
    ...
    resource = build_canonical_storage_hosting_resource()
    trace = build_traceability(
        resource,
        unit=REALIZATION_UNIT,
        forward=(
            resource.resource_id,
            "EC3-B13-U04-VALIDATION-EVIDENCE",
            "EC3-B13-U04-CCE-CERTIFICATION",
            "EC3-B13-U04-COMPLETION-REPORT",
        ),
    )
    validation = validate_storage(resource, trace)
...
def main(argv: list[str] | None = None) -> int:
    """CLI entry: realize EC3-B13-U04, emit evidence, print the determination."""
    parser = argparse.ArgumentParser(
        prog="ec3-b13-u04-realize",
        description="Realize the Universal Infrastructure Storage-Hosting resource.",
    )
    ...
    print(f"[{'PASS' if complete else 'FAIL'}] EC3-B13-U04 → {summary['determination']}")
...
__all__ = [
    ...
    "CANONICAL_TYPE_TAG",
    "CANONICAL_LOCALITY_REF",
    "CANONICAL_DATUM_HOST_REF",
    "CANONICAL_CAPACITY_AMOUNT",
    "CANONICAL_CAPACITY_UNIT",
    "DEFAULT_EVIDENCE_DIR",
    "RealizationResult",
    "build_canonical_storage_hosting_resource",
    "realize",
    "determinism_check",
    "emit_evidence",
    "main",
