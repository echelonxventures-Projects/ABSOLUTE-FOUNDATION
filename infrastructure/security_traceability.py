"""EC3-B13-U08 — Traceability record (No-Orphan closure, GOV-001-T3).

Every realized Security facet records a closed lineage (EC-3-B13-P01 §5), rooted at its
leaf meta-class (**SecurityFacet**) and closing to the frozen spec anchor — binding the
chain **construct → evidence → registry → certification**:

* **Backward:** ``SecurityFacet → INFRASTRUCTURE-013 → INFRASTRUCTURE-005 →
  INFRASTRUCTURE-001 → ARCH-INFRASTRUCTURE-001 → 13-INFRASTRUCTURE@b7e7657``.
* **Substrate:** the frozen foundations ``ENG-001…005`` (EL-1), ``RL-F2`` (policy by
  reference), and the frozen lower-layer security concerns ``IsolationBoundary`` (011),
  ``DATA-014`` / ``SERVICE-014`` / ``APPLICATION-013`` (reused by reference — ISEC-02).
...
from infrastructure.security import FOUNDATION_REUSE, SecurityFacet
...
def build_traceability(
    construct: SecurityFacet, *, unit: str, forward: tuple[str, ...]
) -> TraceabilityRecord:
