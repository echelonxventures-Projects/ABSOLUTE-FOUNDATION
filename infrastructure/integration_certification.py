"""EC3-B13-U10 — Universal Infrastructure Integration certification (CCE CC-1…CC-10 + C1…C7).

Certification is *aggregation, not re-judgment* (TP-01 soundness): every gate below reads
only the outcome of the Integration validation (:mod:`infrastructure.integration_validation`)
...
Two evidence families are produced per dependency construct:
...
* **C1…C7** — the INFRASTRUCTURE-001 §12 Infrastructure-compliance conditions. For this
  unit, **C5** (references use ENG-005; founding structure acyclic — UIL-09 / WF-3) and
  **C7** (no technology/authority/secret — UIL-15) are the **materially-exercised**
...
from infrastructure.integration_meta import INFRASTRUCTURE_COMPLIANCE
from infrastructure.integration_validation import IntegrationValidation
...
class Gate3Coverage(_GateCriterion):
    ...
    required_checks = ("dependency-value-fidelity", "meta-relationships-closed")
...
_COMPLIANCE_CHECKS: dict[str, tuple[str, ...]] = {
    "C1": ("dependency-typed", "dependency-identified"),
    "C2": ("foundation-reuse-integrity",),
    "C3": ("dependency-reference-only", "dependency-endpoints-resolve"),
    "C4": ("authority-boundary", "dependency-reference-only"),
    "C5": ("dependency-downward-acyclic", "meta-relationships-closed", "founding-acyclic"),
    "C6": ("lifecycle-valid",),
...
class IntegrationCertification:
    """The bundled certification outcome for a dependency construct."""
...
def certify_dependency(
    validation: IntegrationValidation,
    *,
    version: str = "1.0.0",
    ledger: CertificationLedger | None = None,
    blueprint_id: str = "InfrastructureDependency",
) -> IntegrationCertification:
    """Certify a validated dependency through the EC-1 CCE engine and ledger it."""
    from engine.certification.contracts import (
    ...
    if ledger is not None:
        ...
        record = CertificationRecord.create(
            ...
            disclosure={"disclosure_id": "ec3-b13-u10-provisional"},
            standard=CERTIFICATION_STANDARD,
            standard_version=CERTIFICATION_STANDARD_VERSION,
            authority=CERTIFICATION_AUTHORITY,
        )
        ledger.append(record)
    return IntegrationCertification(decision=decision, evidence=evidence, compliance=compliance)
