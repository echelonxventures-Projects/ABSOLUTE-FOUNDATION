"""EC3-B13-U07 — Resilience & Availability certification (CCE gates CC-1…CC-10 + C1…C7).

Certification is *aggregation, not re-judgment* (TP-01 soundness): every gate below reads
only the outcome of the Resilience & Availability validation
(:mod:`infrastructure.resilience_validation`) and never re-inspects the construct. It runs
...
* **C1…C7** — the INFRASTRUCTURE-001 §12 Infrastructure-compliance conditions. For this
  unit, **C6** (scaling posture unbounded — UIL-13/WF-9) and **C7** (no technology
...
from infrastructure.resilience_validation import ResilienceValidation
...
class Gate3Coverage(_GateCriterion):
    ...
    required_checks = (
        "infra-resilience-value-fidelity",
...
_COMPLIANCE_CHECKS: dict[str, tuple[str, ...]] = {
    "C1": ("infra-resilience-typed", "infra-resilience-identified-objectbound"),
    "C2": ("foundation-reuse-integrity",),
    "C3": ("foundation-reuse-integrity",),
    "C4": ("non-constitutive",),
    "C5": ("meta-relationships-closed",),
    "C6": ("no-artificial-ceiling", "posture-valid"),
    ...
    *,
    ...
    if ledger is not None:
        ...
        record = CertificationRecord.create(
            ...
            disclosure={"disclosure_id": "ec3-b13-u07-provisional"},
            standard=CERTIFICATION_STANDARD,
            standard_version=CERTIFICATION_STANDARD_VERSION,
            authority=CERTIFICATION_AUTHORITY,
        )
        ledger.append(record)
    return ResilienceCertification(decision=decision, evidence=evidence, compliance=compliance)
