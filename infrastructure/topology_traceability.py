"""EC3-B13-U06 — Traceability record (No-Orphan closure, GOV-001-T3).

Every realized Topology & Distribution construct records a closed lineage
(EC-3-B13-P01 §5), rooted at **its own** leaf meta-class (Topology or Distribution)
and closing to the frozen spec anchor:

* **Backward:** ``<meta-class> → INFRASTRUCTURE-010 → INFRASTRUCTURE-005 →
...
* **Substrate:** the frozen foundations ``ENG-001…005`` (EL-1), ``PL-F2`` (platform
  composition — ITOP-01), ``RL-F2`` (runtime — Distribution lifecycle), ``SF-2`` and
  ``AF-3`` (hosted capabilities — ITOP-03).
...
from infrastructure.topology import FOUNDATION_REUSE, _InfraConstruct
