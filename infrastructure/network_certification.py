"""EC3-B13-U03 — Network Resource certification (CCE gates CC-1…CC-10 + C1…C7).

Certification is *aggregation, not re-judgment* (TP-01 soundness): every gate below reads
only the outcome of the network-layer validation
(:mod:`infrastructure.network_validation`) and never re-inspects the resource. It runs
...
* **C1…C7** — the INFRASTRUCTURE-001 §12 Infrastructure-compliance conditions, decided on
  the same validation evidence. For the Network Resource, **C4** (environments bounded/
  isolated + resources explicit — UIL-07/08; connectivity honors isolation boundaries) and
  **C5** (connectivity uses ENG-005 references + founding acyclic — UIL-09) are the
...
from infrastructure.network_meta import INFRASTRUCTURE_COMPLIANCE
from infrastructure.network_validation import NetworkValidation
...
class Gate1Architecture(_GateCriterion):
    criterion_id = "CC-1"
    description = "Gate 1 Architecture — no orphan (resource traces the frozen spec spine)."
...
#: For the Network Resource, **C4** (environments bounded/isolated + resources explicit —
#: UIL-07/08; connectivity honors isolation boundaries) and **C5** (connectivity uses
#: ENG-005 references + founding acyclic — UIL-09) are the materially-exercised governing
#: conditions. C3's PL-F2/SF-2/AF-3/DF-2 hosting is scoped to the Capability/Distribution/
#: Storage-Hosting units; for Network it is satisfied by connectivity between hosted
#: constructs expressed as typed ENG-005 references, recorded with a scoping note.
_COMPLIANCE_CHECKS: dict[str, tuple[str, ...]] = {
    "C1": ("infra-network-typed", "infra-network-identified-objectbound"),
    "C2": ("foundation-reuse-integrity",),
    "C3": ("infra-network-connectivity-by-reference", "connectivity-by-reference"),
    "C4": (
    ...
    "C6": ("scaling-unbounded",),
    "C7": ("non-constitutive", "technology-independence"),
}

_C3_NOTE = (
    "PL-F2/SF-2/AF-3 delivery + DF-2 data-hosting scoped to the Capability/Distribution/"
    "Storage-Hosting units (INFRASTRUCTURE-006/009/010); a Network Resource hosts "
    "connectivity between hosted constructs as typed ENG-005 references (ICNW-01)"
)


@dataclass(frozen=True, slots=True)
class NetworkComplianceReport:
...
def evaluate_network_compliance(validation: NetworkValidation) -> NetworkComplianceReport:
    """Decide the C1…C7 compliance of the validated resource on validation evidence."""
    ...
    return NetworkComplianceReport(
...
@dataclass(frozen=True, slots=True)
class NetworkCertification:
    """The bundled outcome of certifying a validated Network Resource."""

    decision: CertificationDecision
    evidence: CertificationEvidence
    ledger_entry: CertificationLedgerEntry
    ledger: CertificationLedger
    compliance: NetworkComplianceReport
...
def certify_network(
    ...
    *,
    ...
    """Run the CCE ten gates + C1…C7 over a validated resource and ledger the record.

    Reuses the CERTIFIED EC-1 certification engine and append-only ledger. The result is
    CERTIFIED iff all ten gates close, the ledger chain is intact, and the resource is
    ...
    compliance = evaluate_network_compliance(validation)
    return NetworkCertification(
        decision=decision,
