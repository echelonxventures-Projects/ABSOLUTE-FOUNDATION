"""EC3-B13-U09 — Traceability record (No-Orphan closure, GOV-001-T3).

Every realized Governance construct records a closed lineage (EC-3-B13-P01 §5), rooted at
its leaf meta-class (**GovernanceFacet**) and closing to the frozen spec anchor — binding
the chain **construct → evidence → registry → certification**:

* **Backward:** ``GovernanceFacet → INFRASTRUCTURE-014 → INFRASTRUCTURE-005 →
  INFRASTRUCTURE-001 → ARCH-INFRASTRUCTURE-001 → 13-INFRASTRUCTURE@b7e7657``.
* **Substrate:** the frozen foundations ``ENG-001…005`` (EL-1), ``ENG-000`` (custodian),
  ``RL-F2`` (policy by reference), and the frozen concerns ``UIMM-CONF`` / ``UITX`` /
  ``UCI-001`` / ``REG-AUTO-001`` (reused by reference — IGOV-05).
...
from infrastructure.governance import FOUNDATION_REUSE, GovernanceFacet
...
def build_traceability(
    construct: GovernanceFacet, *, unit: str, forward: tuple[str, ...]
) -> TraceabilityRecord:
