"""EC3-B13-U08 — Infrastructure Security certification (CCE gates CC-1…CC-10 + C1…C7).

Certification is *aggregation, not re-judgment* (TP-01 soundness): every gate below reads
only the outcome of the Security validation (:mod:`infrastructure.security_validation`) and
never re-inspects the construct. It runs through the **CERTIFIED EC-1 Certification
...
* **C1…C7** — the INFRASTRUCTURE-001 §12 Infrastructure-compliance conditions. For this
  unit, **C4** (evaluative, non-enforcing — WF-10/UIL-14) and **C7** (no
  technology/authority/secret — UIL-15/ISEC-03/05/06) are the **materially-exercised**
...
from infrastructure.security_validation import SecurityValidation
...
_COMPLIANCE_CHECKS: dict[str, tuple[str, ...]] = {
    "C1": ("infra-security-typed", "infra-security-identified-objectbound"),
    "C2": ("foundation-reuse-integrity",),
    "C3": ("security-evaluates-objectbound",),
    "C4": ("security-evaluative-nonenforcing", "authority-boundary"),
    ...
    *,
    ...
    if ledger is not None:
        ...
        record = CertificationRecord.create(
            ...
            disclosure={"disclosure_id": "ec3-b13-u08-provisional"},
            standard=CERTIFICATION_STANDARD,
            standard_version=CERTIFICATION_STANDARD_VERSION,
            authority=CERTIFICATION_AUTHORITY,
        )
        ledger.append(record)
    return SecurityCertification(decision=decision, evidence=evidence, compliance=compliance)
