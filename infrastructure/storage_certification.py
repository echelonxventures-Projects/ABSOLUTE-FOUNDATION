"""EC3-B13-U04 — Storage-Hosting Resource certification (CCE gates CC-1…CC-10 + C1…C7).

Certification is *aggregation, not re-judgment* (TP-01 soundness): every gate below reads
only the outcome of the storage-layer validation
(:mod:`infrastructure.storage_validation`) and never re-inspects the resource. It runs
...
* **C1…C7** — the INFRASTRUCTURE-001 §12 Infrastructure-compliance conditions, decided on
  the same validation evidence. For the Storage-Hosting Resource, **C3** (hosted data
  DF-2 by reference — UIL-11) and **C4** (resources explicit + data placement honors
  isolation boundaries — UIL-07/08) are the **materially-exercised** governing conditions.
...
from infrastructure.storage_meta import INFRASTRUCTURE_COMPLIANCE
from infrastructure.storage_validation import StorageValidation
...
#: For the Storage-Hosting Resource, **C3** (hosted data DF-2 by reference — UIL-11) and
#: **C4** (resources explicit + data placement honors isolation boundaries — UIL-07/08)
#: are the materially-exercised governing conditions. C3's PL-F2/SF-2/AF-3 *delivery* half
#: is scoped to the Distribution unit; for Storage-Hosting the DF-2 *data-hosting* half is
#: materially satisfied by the resource hosting the frozen DATA-010 datum by reference.
_COMPLIANCE_CHECKS: dict[str, tuple[str, ...]] = {
    "C1": ("infra-storage-typed", "infra-storage-identified-objectbound"),
    "C2": ("foundation-reuse-integrity",),
    "C3": ("infra-storage-hosts-data-by-reference", "hosting-by-reference"),
    "C4": ("infra-storage-declares-capacity-locality", "infra-storage-honors-boundaries"),
    "C5": ("founding-acyclic", "meta-relationships-closed"),
    "C6": ("infra-storage-hosts-data-by-reference", "scaling-unbounded"),
    "C7": ("non-constitutive", "technology-independence"),
}

_C3_NOTE = (
    "PL-F2/SF-2/AF-3 delivery scoped to the Distribution unit (INFRASTRUCTURE-010/012); a "
    "Storage-Hosting Resource materially satisfies the DF-2 data-hosting half by hosting "
    "the frozen DATA-010 datum by reference (ISTO-01 / UIL-11), redefining no data concern"
)


@dataclass(frozen=True, slots=True)
class StorageComplianceReport:
...
def evaluate_storage_compliance(validation: StorageValidation) -> StorageComplianceReport:
...
class StorageCertification:
    """The bundled outcome of certifying a validated Storage-Hosting Resource."""

    decision: CertificationDecision
    evidence: CertificationEvidence
    ledger_entry: CertificationLedgerEntry
    ledger: CertificationLedger
    compliance: StorageComplianceReport
