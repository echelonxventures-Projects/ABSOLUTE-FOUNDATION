"""EC3-B13-U01 — Infrastructure Capability certification (CCE gates CC-1…CC-10 + C1…C7).

Certification is *aggregation, not re-judgment* (TP-01 soundness): every gate below
reads only the outcome of the capability-layer validation
(:mod:`infrastructure.capability_validation`) and never re-inspects the capability. It
...
* **C1…C7** — the INFRASTRUCTURE-001 §12 Infrastructure-compliance conditions, decided
  on the same validation evidence, deterministically and non-coercively (UIL-14).
...
from infrastructure.capability_meta import INFRASTRUCTURE_COMPLIANCE
from infrastructure.capability_validation import CapabilityValidation
...
class Gate1Architecture(_GateCriterion):
    criterion_id = "CC-1"
    description = "Gate 1 Architecture — no orphan (capability traces the frozen spec spine)."
...
#: C4 (environment/resource/distribution) is scoped to the Environment/Resource/
#: Topology-Distribution units (INFRASTRUCTURE-011/007/008/009/010); for the Capability
#: it is satisfied by the capability's own explicit typed structure (type + kind +
#: resolvable references), recorded with a scoping note.
_COMPLIANCE_CHECKS: dict[str, tuple[str, ...]] = {
    "C1": ("infra-capability-typed", "infra-capability-identified-objectbound"),
    "C2": ("foundation-reuse-integrity",),
    "C3": (
        "infra-capability-reuses-platform",
        "infra-capability-enables-frozen-construct",
        "hosting-by-reference",
    ),
    "C4": ("infra-capability-typed", "infra-capability-classified"),
    "C5": ("founding-acyclic", "meta-relationships-closed"),
    "C6": ("behavior-by-reference", "scaling-unbounded"),
    "C7": ("non-constitutive", "technology-independence"),
}

_C4_NOTE = (
    "environment/resource/distribution scoped to INFRASTRUCTURE-011/007/008/009/010 units; "
    "capability structure explicit (typed + classified)"
)
...
def evaluate_capability_compliance(validation: CapabilityValidation) -> CapabilityComplianceReport:
...
class CapabilityCertification:
    """The bundled outcome of certifying a validated Infrastructure Capability."""

    decision: CertificationDecision
    evidence: CertificationEvidence
    ledger_entry: CertificationLedgerEntry
    ledger: CertificationLedger
    compliance: CapabilityComplianceReport
...
def certify_capability(
    ...
    *,
    ...
    CERTIFIED iff all ten gates close, the ledger chain is intact, and the capability is
