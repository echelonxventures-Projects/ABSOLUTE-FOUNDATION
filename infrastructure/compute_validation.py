"""EC3-B13-U02 — Compute Resource validation (meta-validity WF + UIL conformance).

This module proves a realized :class:`~infrastructure.compute.ComputeResource` is
...
The checks are pure predicates over an immutable :class:`ComputeValidationSubject` (the
...
capacity and locality) the **governing, materially-exercised** obligation for this unit.
"""
...
from infrastructure.compute import INFRA_COMPUTE_ID_PREFIX, ComputeResource
from infrastructure.compute_meta import (
    ADMITTED_META_RELATIONSHIPS,
    INFRASTRUCTURE_META_CLASS,
    InfrastructureState,
)
from infrastructure.compute_traceability import TraceabilityRecord

#: The blueprint id a Compute Resource realizes (its meta-class).
COMPUTE_BLUEPRINT_ID = INFRASTRUCTURE_META_CLASS
...
_ID_PREFIX = f"{INFRA_COMPUTE_ID_PREFIX}-"


@dataclass(frozen=True, slots=True)
class ComputeValidationSubject:
    """A normalized, immutable projection of a compute resource that checks evaluate.
    ...
    hosts_execution_by_reference: bool
    ...
    @classmethod
    def from_resource(
        cls, resource: ComputeResource, trace: TraceabilityRecord
        ...
        return cls(
            ...
            execution_host_ref=resource.execution_host_ref,
            ...
            hosts_execution_by_reference=resource.hosts_execution_by_reference(),
...
# ---------------------------------------------------------------------------
# Infrastructure-layer validation checks (each maps to explicit WF*/UIL*/ICMP* obligations)
# ---------------------------------------------------------------------------


class ComputeTypedCheck(ValidationCheck):
    """UIL-03 / ICMP-02 / C1 — the resource is classified by a non-empty ENG-004 Compute Class."""

    check_id = "infra-compute-typed"
    severity = Severity.BLOCKING
    description = "Compute resource bears a non-empty ENG-004 Compute Class type_tag (UIL-03)."

    def evaluate(self, subject: ComputeValidationSubject) -> ValidationFinding:
        if not subject.type_tag.strip():
            return self._failed("compute resource is untyped (UIL-03)")
        return self._passed(type_tag=subject.type_tag)


class ComputeIdentifiedCheck(ValidationCheck):
    """UIL-04/05 / ICMP-02 / C1 — the resource is identified (ENG-001) and object-borne."""

    check_id = "infra-compute-identified-objectbound"
    severity = Severity.BLOCKING
...
class ComputeValueFidelityCheck(ValidationCheck):
    """ENG-003 / C3 — the resource core round-trips through the EC-1 canonical encoding."""

    check_id = "infra-compute-value-fidelity"
    severity = Severity.BLOCKING
...
class ComputeDeclaresCapacityLocalityCheck(ValidationCheck):
    """WF-5 / UIL-08 / ICMP-02 / C4 — every Resource declares capacity and locality.

    THE governing, materially-exercised obligation for the Compute Resource: a Resource
    that omits either a quantified capacity or a locality reference is ill-formed.
    """

    check_id = "infra-compute-declares-capacity-locality"
    severity = Severity.BLOCKING
    description = "Compute resource declares a quantified capacity and a locality (WF-5 / UIL-08)."

    def evaluate(self, subject: ComputeValidationSubject) -> ValidationFinding:
        if subject.capacity_amount < 0 or not subject.capacity_unit.strip():
            return self._failed("resource omits a quantified capacity (WF-5 / ICMP-02)")
        if not subject.located_by_reference or not subject.locality_ref.strip():
            return self._failed("resource omits its locality (WF-5 / ICMP-02)")
...
class ComputeHostsExecutionByReferenceCheck(ValidationCheck):
    """ICMP-01 / WF-2 / UIL-10 / C6 — the resource hosts RL-F2 execution by ENG-005 reference."""

    check_id = "infra-compute-hosts-execution-by-reference"
    severity = Severity.BLOCKING
    description = "Resource hosts the frozen RL-F2 execution concern by ENG-005 ref (ICMP-01)."

    def evaluate(self, subject: ComputeValidationSubject) -> ValidationFinding:
        if not subject.hosts_execution_by_reference or not subject.execution_host_ref.strip():
            return self._failed("resource hosts no RL-F2 execution by reference (ICMP-01 / UIL-10)")
        return self._passed(execution_host_ref=subject.execution_host_ref)


class MetaClassSingleCheck(ValidationCheck):
    """WF-1 — the resource instantiates exactly one leaf meta-class (ComputeResource)."""

    check_id = "meta-class-single"
    severity = Severity.BLOCKING
    description = "Resource instantiates exactly the ComputeResource meta-class (WF-1)."

    def evaluate(self, subject: ComputeValidationSubject) -> ValidationFinding:
        if subject.meta_class != INFRASTRUCTURE_META_CLASS:
            return self._failed(
                "meta-class is not ComputeResource (WF-1)", meta_class=subject.meta_class
            )
...
class FoundingAcyclicCheck(ValidationCheck):
    """WF-3 / UIL-09 / ICMP-03 — the founding graph is acyclic (placement acyclic)."""

    check_id = "founding-acyclic"
    severity = Severity.BLOCKING
    description = "The resource's founding graph is acyclic (WF-3 / UIL-09 / ICMP-03)."
...
class ResourceKindCheck(ValidationCheck):
    ...
    check_id = "infra-compute-is-resource"
    severity = Severity.BLOCKING
...
class HostingByReferenceCheck(ValidationCheck):
    """UIL-10 / ICMP-01 / C6 — hosted execution + locality bound by ENG-005 reference."""

    check_id = "hosting-by-reference"
    severity = Severity.BLOCKING
    description = "Hosted execution + locality bound by ENG-005 reference (UIL-10 / WF-2)."

    def evaluate(self, subject: ComputeValidationSubject) -> ValidationFinding:
        if not subject.execution_host_ref.strip() or not subject.locality_ref.strip():
            return self._failed("hosting/locality reference missing (UIL-10 / WF-2)")
        return self._passed(
            execution_host_ref=subject.execution_host_ref, locality_ref=subject.locality_ref
        )


class ScalingUnboundedCheck(ValidationCheck):
    """UIL-13 / ICMP-04 / C6 — the resource declares no artificial capacity ceiling."""
    ...
    def evaluate(self, subject: ComputeValidationSubject) -> ValidationFinding:
        if not subject.declares_no_artificial_ceiling:
            return self._failed("resource declares an artificial ceiling (UIL-13 / ICMP-04)")
        return self._passed()


class TechnologyIndependenceCheck(ValidationCheck):
    """UIL-12 / UIL-15 / ICMP-05 / C7 — no hardware/VM/container/orchestrator/vendor selected."""

    check_id = "technology-independence"
    severity = Severity.BLOCKING
    description = "No concrete hardware/technology/orchestrator/vendor selected (UIL-12 / UIL-15)."

    def evaluate(self, subject: ComputeValidationSubject) -> ValidationFinding:
        if subject.selects_technology:
            return self._failed("a concrete technology was selected (UIL-12 / UIL-15 / ICMP-05)")
        return self._passed()


class NonConstitutiveCheck(ValidationCheck):
    """UIL-14/15 / ICMP-05 / C7 — confers no authority, enacts no enforcement, embeds no secret."""
...
def compute_checks() -> tuple[ValidationCheck, ...]:
    """The full compute-resource validation suite (deterministically ordered)."""
    return (
        ComputeTypedCheck(),
        ComputeIdentifiedCheck(),
        ComputeValueFidelityCheck(),
        ComputeDeclaresCapacityLocalityCheck(),
        ComputeHostsExecutionByReferenceCheck(),
...
def validate_compute(
    resource: ComputeResource, trace: TraceabilityRecord, *, strict: bool = False
) -> ComputeValidation:
    ...
    """
    subject = ComputeValidationSubject.from_resource(resource, trace)
    engine = ValidationEngine(compute_checks())
    report = engine.validate(subject)
    evidence = build_validation_evidence(report)
    decision = enforce_acceptance(report, strict=strict)
    return ComputeValidation(report=report, evidence=evidence, decision=decision)
