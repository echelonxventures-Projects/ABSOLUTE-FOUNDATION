"""EC3-B13-U02 — Traceability record (No-Orphan closure, GOV-001-T3).

Every realized Compute Resource records a closed lineage (EC-3-B13-P01 §5):

* **Backward:** ``ComputeResource → INFRASTRUCTURE-007 → INFRASTRUCTURE-005 →
  INFRASTRUCTURE-001 → ARCH-INFRASTRUCTURE-001 → 13-INFRASTRUCTURE@b7e7657``.
* **Substrate:** the frozen foundations ``ENG-001…005`` (EL-1) and ``RL-F2`` (the hosted
  execution concern) — referenced, not redefined (UIL-02).
* **Forward:** the realized Compute Resource construct + its validation/certification
  evidence.
...
reference — the same closed-lineage structure, re-parameterized for the Compute concern.
"""
...
from infrastructure.compute import FOUNDATION_REUSE, ComputeResource
...
class TraceabilityRecord:
    """A deterministic, closed No-Orphan lineage record for a realized compute resource."""
...
def build_traceability(
    resource: ComputeResource, *, unit: str, forward: tuple[str, ...]
) -> TraceabilityRecord:
