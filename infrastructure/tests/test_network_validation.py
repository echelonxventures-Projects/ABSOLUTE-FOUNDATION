"""EC3-B13-U03 — Validation tests (EC-1 ValidationEngine PASS + WF + UIL + VC).

Fail-branch coverage is achieved by projecting a valid network resource into a
:class:`NetworkValidationSubject` and flipping a single field (``dataclasses.replace``)
...
from infrastructure.network_meta import InfrastructureState
from infrastructure.network_traceability import build_traceability
from infrastructure.network_validation import (
...
def _resource(**overrides):
    kwargs = dict(type_tag="ucos.demo.network", locality_ref=LOCALITY)
    kwargs.update(overrides)
    return make_network_resource(kwargs.pop("type_tag"), kwargs.pop("locality_ref"), **kwargs)


def _trace(resource):
    return build_traceability(resource, unit="EC3-B13-U03", forward=(resource.resource_id,))


def _subject(**overrides):
    r = _resource()
    subject = NetworkValidationSubject.from_resource(r, _trace(r))
    return replace(subject, **overrides) if overrides else subject


def _fails(subject, check_id: str) -> bool:
    report = ValidationEngine(network_checks()).validate(subject)
...
def test_every_check_is_blocking_and_passes():
    ...
    assert counts["total"] == len(network_checks())
    assert counts["blocking_failed"] == 0


def test_meta_validity_wf_checks_hold():
    r = _resource()
    passed = {f.check_id: f.passed for f in validate_network(r, _trace(r)).report.findings}
    assert passed["meta-class-single"]  # WF-1
    assert passed["infra-network-connectivity-by-reference"]  # WF-2
    assert passed["founding-acyclic"]  # WF-3
    assert passed["infra-network-declares-capacity-locality"]  # WF-5 — a governing rule
...
def test_uil_conformance_checks_present_and_pass():
    ...
    passed = {f.check_id: f.passed for f in validate_network(r, _trace(r)).report.findings}
    for cid in (
        "infra-network-typed",  # UIL-03
        "infra-network-identified-objectbound",  # UIL-04/05
        "infra-network-value-fidelity",  # ENG-003
        "infra-network-declares-capacity-locality",  # UIL-08 / WF-5
        "infra-network-declares-connected-endpoints",  # ICNW-02
        "infra-network-connectivity-by-reference",  # ICNW-01 / UIL-09
        "infra-network-honors-boundaries",  # ICNW-03 / UIL-07 — the distinguishing law
        "infra-network-is-resource",  # WF-5
        "foundation-reuse-integrity",  # UIL-02 / VC-5
        "connectivity-by-reference",  # UIL-09
...
def test_subject_projection_is_deterministic():
    r = _resource()
    assert NetworkValidationSubject.from_resource(
        r, _trace(r)
    ) == NetworkValidationSubject.from_resource(r, _trace(r))


def test_active_resource_still_validates():
    r = _resource(state=InfrastructureState.ACTIVE)
    assert validate_network(r, _trace(r)).accepted
...
def test_untyped_resource_fails_typed_gate():
    assert _fails(_subject(type_tag=""), "infra-network-typed")


def test_bad_identity_fails_identified_gate():
    assert _fails(_subject(target_id="NOT-A-NETWORK-id"), "infra-network-identified-objectbound")


def test_missing_digest_fails_identified_gate():
    assert _fails(_subject(value_digest=""), "infra-network-identified-objectbound")


def test_missing_mandatory_attribute_fails_identified_gate():
    assert _fails(
        _subject(declares_mandatory_attributes=False),
        "infra-network-identified-objectbound",
    )


def test_bad_digest_fails_value_fidelity_gate():
    assert _fails(_subject(value_digest="xyz"), "infra-network-value-fidelity")


def test_negative_capacity_fails_capacity_locality_gate():
    assert _fails(_subject(capacity_amount=-1), "infra-network-declares-capacity-locality")


def test_empty_capacity_unit_fails_capacity_locality_gate():
    assert _fails(_subject(capacity_unit="   "), "infra-network-declares-capacity-locality")


def test_missing_locality_fails_capacity_locality_gate():
    assert _fails(
        _subject(locality_ref="   "),
        "infra-network-declares-capacity-locality",
    )
...
def test_no_endpoints_fails_connected_endpoints_gate():
    assert _fails(_subject(link_count=0), "infra-network-declares-connected-endpoints")


def test_unresolved_endpoints_fails_connected_endpoints_gate():
    assert _fails(
        _subject(declares_connected_endpoints=False),
        "infra-network-declares-connected-endpoints",
    )


def test_connectivity_not_by_reference_fails_connectivity_gate():
    assert _fails(
        _subject(connectivity_by_reference=False),
        "infra-network-connectivity-by-reference",
    )


def test_unhonored_boundary_fails_boundary_gate():
    assert _fails(
        _subject(honors_isolation_boundaries=False),
        "infra-network-honors-boundaries",
    )
...
def test_non_resource_fails_resource_kind_gate():
    assert _fails(_subject(is_resource=False), "infra-network-is-resource")


def test_evaluative_facet_fails_resource_kind_gate():
    assert _fails(_subject(is_evaluative_facet=True), "infra-network-is-resource")
...
def test_missing_connectivity_reference_fails_connectivity_reference_gate():
    assert _fails(_subject(connectivity_by_reference=False), "connectivity-by-reference")
    assert _fails(_subject(locality_ref=""), "connectivity-by-reference")
...
def test_unrooted_lineage_fails_traceability_gate():
    assert _fails(
        _subject(provenance_chain=("INFRASTRUCTURE-005", "NetworkResource")),
        "traceability-rooted",
    )
...
def test_lineage_without_anchor_fails_traceability_gate():
    assert _fails(
        _subject(provenance_chain=("NetworkResource", "INFRASTRUCTURE-001")),
        "traceability-rooted",
    )


def test_real_technology_bearing_resource_is_rejected():
    r = _resource(target_ref="ENG-005:INFRASTRUCTURE-007:kubernetes.service.endpoint")
    result = validate_network(r, _trace(r))
