"""EC3-B13-U04 — Realization + determinism tests (VC-4, AC-1…7, VC-1…5, evidence, CLI)."""
...
from infrastructure.storage_certification import StorageComplianceReport, certify_storage
from infrastructure.storage_realize import (
...
from infrastructure.storage_validation import validate_storage
...
def test_canonical_resource_is_the_storage_foundation():
    r = build_canonical_storage_hosting_resource()
    assert r.type_tag == CANONICAL_TYPE_TAG
    assert r.locality_ref == CANONICAL_LOCALITY_REF
    assert r.placements[0].datum_ref == CANONICAL_DATUM_HOST_REF
    assert r.capacity.unit == CANONICAL_CAPACITY_UNIT
    assert r.meta_class == "StorageHostingResource"
...
def test_meta_validity_and_uil_conformance_complete():
    ...
    assert set(mv) == {"WF-1", "WF-2", "WF-3", "WF-5", "WF-7", "WF-11", "WF-12"}
...
def test_certification_gates_and_compliance_roll_up():
    ...
    assert all(result.storage_compliance().values())  # C1…C7
...
def test_bundle_shape_is_deterministic_and_complete():
    ...
    assert bundle["unit"] == "EC3-B13-U04"
    assert bundle["meta_class"] == "StorageHostingResource"
...
def test_cli_main_returns_zero(tmp_path, capsys):
    ...
    assert rc == 0
    assert "[PASS] EC3-B13-U04 → COMPLETE" in capsys.readouterr().out
...
def test_cli_main_reports_failure_when_not_complete(tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr("infrastructure.storage_realize.emit_evidence", _failing_emit)
    rc = main(["--evidence-dir", str(tmp_path)])
    assert rc == 1
    assert "[FAIL] EC3-B13-U04 → NOT COMPLETE" in capsys.readouterr().out
...
def test_determination_not_complete_for_rejected_resource():
    ...
    bad = make_storage_hosting_resource("t", "ENG-005:INFRASTRUCTURE-011:locality.foundation",
                                        datum_ref="ENG-005:DF-2:postgres.table")
    trace = build_traceability(bad, unit="EC3-B13-U04", forward=(bad.resource_id,))
    validation = validate_storage(bad, trace)
...
def test_traceability_record_fingerprint_and_flags():
    r = make_storage_hosting_resource("t", CANONICAL_LOCALITY_REF)
    trace = build_traceability(r, unit="EC3-B13-U04", forward=(r.resource_id,))
...
def test_determination_complete_with_conditions_when_certification_degraded():
    ...
    r = build_canonical_storage_hosting_resource()
    trace = build_traceability(r, unit="EC3-B13-U04", forward=(r.resource_id,))
    validation = validate_storage(r, trace)
    certification = certify_storage(validation, version=UNIT_VERSION)
    degraded_compliance = StorageComplianceReport(
