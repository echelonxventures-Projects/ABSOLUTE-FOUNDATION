"""EC3-B13-U02 — Certification tests (CCE CC-1…CC-10 + Infrastructure C1…C7 + ledger).
...
from infrastructure.compute_meta import CCE_GATES
from infrastructure.compute_traceability import build_traceability
from infrastructure.compute_validation import compute_checks, validate_compute

LOCALITY = "ENG-005:INFRASTRUCTURE-011:locality.foundation"

_ALL_CHECKS = tuple(c.check_id for c in compute_checks())


def _validate(resource):
    trace = build_traceability(resource, unit="EC3-B13-U02", forward=(resource.resource_id,))
    return validate_compute(resource, trace)


def _good_subject(**overrides) -> CertificationSubject:
    """A crafted certification subject where every gate would otherwise close."""
    base = dict(
        target_id="UCOS-INFRA-COMPUTE-ucos.demo.compute-0000000000000000",
        blueprint_id="ComputeResource",
        version="1.0.0",
...
def test_cce_ten_gates_all_close_and_certify():
    cert = certify_compute(_validate(make_compute_resource("t", LOCALITY)), version="1.0.0")
...
def test_infrastructure_compliance_c1_c7_all_hold():
    compliance = evaluate_compute_compliance(_validate(make_compute_resource("t", LOCALITY)))
...
def test_certification_id_is_deterministic():
    a = certify_compute(_validate(make_compute_resource("t", LOCALITY)), version="1.0.0")
    b = certify_compute(_validate(make_compute_resource("t", LOCALITY)), version="1.0.0")
    assert a.decision.certification_id == b.decision.certification_id


def test_compliance_report_is_directly_constructible():
    report = ComputeComplianceReport(target_id="x", conditions=())
...
def test_non_compliant_resource_is_not_certified():
    ...
    cert = certify_compute(
        _validate(make_compute_resource("t", LOCALITY, execution_host_ref="ENG-005:RL-F2:kubernetes")),
        version="1.0.0",
    )
...
def test_supplied_ledger_is_reused():
    ...
    certify_compute(_validate(make_compute_resource("t", LOCALITY)), version="1.0.0", ledger=ledger)
    certify_compute(
        _validate(make_compute_resource("t", "ENG-005:INFRASTRUCTURE-011:locality.other")),
        version="1.0.0",
        ledger=ledger,
    )
    assert len(ledger) == 2
