"""EC3-B13-U04 — Certification tests (CCE CC-1…CC-10 + Infrastructure C1…C7 + ledger).
...
from infrastructure.storage_certification import (
    Gate1Architecture,
    Gate2Dependencies,
    Gate3Coverage,
    Gate4Validation,
...
from infrastructure.storage_meta import CCE_GATES
from infrastructure.storage_traceability import build_traceability
from infrastructure.storage_validation import storage_checks, validate_storage

LOCALITY = "ENG-005:INFRASTRUCTURE-011:locality.foundation"

_ALL_CHECKS = tuple(c.check_id for c in storage_checks())


def _validate(resource):
    trace = build_traceability(resource, unit="EC3-B13-U04", forward=(resource.resource_id,))
    return validate_storage(resource, trace)


def _good_subject(**overrides) -> CertificationSubject:
    """A crafted certification subject where every gate would otherwise close."""
    base = dict(
        target_id="UCOS-INFRA-STORAGE-ucos.demo.storage-0000000000000000",
        blueprint_id="StorageHostingResource",
        version="1.0.0",
...
def test_cce_ten_gates_all_close_and_certify():
    cert = certify_storage(_validate(make_storage_hosting_resource("t", LOCALITY)), version="1.0.0")
...
def test_infrastructure_compliance_c1_c7_all_hold():
    compliance = evaluate_storage_compliance(_validate(make_storage_hosting_resource("t", LOCALITY)))
    ...
    # C3 carries the scoping note (PL-F2/SF-2/AF-3 delivery is a downstream Distribution unit).
...
def test_certification_id_is_deterministic():
    a = certify_storage(_validate(make_storage_hosting_resource("t", LOCALITY)), version="1.0.0")
    b = certify_storage(_validate(make_storage_hosting_resource("t", LOCALITY)), version="1.0.0")
    assert a.decision.certification_id == b.decision.certification_id


def test_compliance_report_is_directly_constructible():
    report = StorageComplianceReport(target_id="x", conditions=())
...
def test_non_compliant_resource_is_not_certified():
    # A technology-selecting resource fails validation → gates fail-closed → NOT-CERTIFIED.
    cert = certify_storage(
        _validate(make_storage_hosting_resource("t", LOCALITY, datum_ref="ENG-005:DF-2:postgres.tbl")),
        version="1.0.0",
    )
...
def test_supplied_ledger_is_reused():
    ...
    certify_storage(_validate(make_storage_hosting_resource("t", LOCALITY)), version="1.0.0", ledger=ledger)
    certify_storage(
        _validate(make_storage_hosting_resource("t", "ENG-005:INFRASTRUCTURE-011:locality.other")),
        version="1.0.0",
        ledger=ledger,
    )
    assert len(ledger) == 2
