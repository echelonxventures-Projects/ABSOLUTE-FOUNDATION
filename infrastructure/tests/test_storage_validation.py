"""EC3-B13-U04 — Validation tests (EC-1 ValidationEngine PASS + WF + UIL + VC).

Fail-branch coverage is achieved by projecting a valid storage resource into a
:class:`StorageValidationSubject` and flipping a single field (``dataclasses.replace``)
...
from infrastructure.storage_meta import InfrastructureState
from infrastructure.storage_traceability import build_traceability
from infrastructure.storage_validation import (
...
def _trace(resource):
    return build_traceability(resource, unit="EC3-B13-U04", forward=(resource.resource_id,))


def _subject(**overrides):
    r = _resource()
    subject = StorageValidationSubject.from_resource(r, _trace(r))
...
def test_meta_validity_wf_checks_hold():
    r = _resource()
    passed = {f.check_id: f.passed for f in validate_storage(r, _trace(r)).report.findings}
    assert passed["meta-class-single"]  # WF-1
    assert passed["meta-constraints"]  # WF-2
    assert passed["founding-acyclic"]  # WF-3
    assert passed["infra-storage-declares-capacity-locality"]  # WF-5 — governing rule
    assert passed["infra-storage-hosts-data-by-reference"]  # WF-7 — the storage rule
...
def test_uil_conformance_checks_present_and_pass():
    ...
    passed = {f.check_id: f.passed for f in validate_storage(r, _trace(r)).report.findings}
    for cid in (
        "infra-storage-typed",  # UIL-03
        "infra-storage-identified-objectbound",  # UIL-04/05
        "infra-storage-value-fidelity",  # ENG-003
        "infra-storage-declares-capacity-locality",  # UIL-08 / WF-5
        "infra-storage-hosts-data-by-reference",  # ISTO-01 / UIL-11 / WF-7
        "infra-storage-honors-boundaries",  # ISTO-03 / UIL-07
        "infra-storage-is-resource",  # WF-5
        "foundation-reuse-integrity",  # UIL-02 / VC-5
        "hosting-by-reference",  # UIL-11
        "scaling-unbounded",  # UIL-13
        "technology-independence",  # UIL-15
...
def test_subject_projection_is_deterministic():
    r = _resource()
    assert StorageValidationSubject.from_resource(
        r, _trace(r)
    ) == StorageValidationSubject.from_resource(r, _trace(r))


def test_active_resource_still_validates():
    r = _resource(state=InfrastructureState.ACTIVE)
    assert validate_storage(r, _trace(r)).accepted
...
def test_untyped_resource_fails_typed_gate():
    assert _fails(_subject(type_tag=""), "infra-storage-typed")


def test_bad_identity_fails_identified_gate():
    assert _fails(_subject(target_id="NOT-A-STORAGE-id"), "infra-storage-identified-objectbound")


def test_missing_digest_fails_identified_gate():
    assert _fails(_subject(value_digest=""), "infra-storage-identified-objectbound")


def test_missing_mandatory_attribute_fails_identified_gate():
    assert _fails(
        _subject(declares_mandatory_attributes=False),
        "infra-storage-identified-objectbound",
    )


def test_bad_digest_fails_value_fidelity_gate():
    assert _fails(_subject(value_digest="xyz"), "infra-storage-value-fidelity")


def test_negative_capacity_fails_capacity_locality_gate():
    assert _fails(_subject(capacity_amount=-1), "infra-storage-declares-capacity-locality")


def test_empty_capacity_unit_fails_capacity_locality_gate():
    assert _fails(_subject(capacity_unit="   "), "infra-storage-declares-capacity-locality")


def test_missing_locality_fails_capacity_locality_gate():
    assert _fails(
        _subject(located_by_reference=False, locality_ref="   "),
        "infra-storage-declares-capacity-locality",
    )
...
def test_missing_hosted_data_fails_hosts_data_gate():
    assert _fails(
        _subject(hosts_data_by_reference=False),
        "infra-storage-hosts-data-by-reference",
    )
    assert _fails(_subject(placement_count=0), "infra-storage-hosts-data-by-reference")


def test_unhonored_boundary_fails_boundaries_gate():
    assert _fails(_subject(honors_isolation_boundaries=False), "infra-storage-honors-boundaries")
...
def test_non_resource_fails_resource_kind_gate():
    assert _fails(_subject(is_resource=False), "infra-storage-is-resource")


def test_evaluative_facet_fails_resource_kind_gate():
    assert _fails(_subject(is_evaluative_facet=True), "infra-storage-is-resource")
...
def test_missing_hosting_reference_fails_hosting_gate():
    assert _fails(_subject(hosts_data_by_reference=False), "hosting-by-reference")
...
def test_unrooted_lineage_fails_traceability_gate():
    assert _fails(
        _subject(provenance_chain=("INFRASTRUCTURE-005", "StorageHostingResource")),
        "traceability-rooted",
    )
...
def test_lineage_without_anchor_fails_traceability_gate():
    assert _fails(
        _subject(provenance_chain=("StorageHostingResource", "INFRASTRUCTURE-001")),
        "traceability-rooted",
    )


def test_real_technology_bearing_resource_is_rejected():
    r = _resource(datum_ref="ENG-005:DF-2:mysql.table")
    result = validate_storage(r, _trace(r))
