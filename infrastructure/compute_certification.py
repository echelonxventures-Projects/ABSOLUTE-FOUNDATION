"""EC3-B13-U02 — Compute Resource certification (CCE gates CC-1…CC-10 + C1…C7).

Certification is *aggregation, not re-judgment* (TP-01 soundness): every gate below reads
only the outcome of the compute-layer validation
(:mod:`infrastructure.compute_validation`) and never re-inspects the resource. It runs
...
* **C1…C7** — the INFRASTRUCTURE-001 §12 Infrastructure-compliance conditions, decided on
  the same validation evidence. For the Compute Resource, **C4** (resources explicit —
  UIL-08) and **C6** (hosts RL-F2 execution by reference + unbounded capacity — UIL-10/13)
  are the **materially-exercised** governing conditions.
...
from infrastructure.compute_meta import INFRASTRUCTURE_COMPLIANCE
from infrastructure.compute_validation import ComputeValidation
...
#: For the Compute Resource, **C4** (resources explicit — UIL-08) and **C6** (hosts RL-F2
#: execution by reference + unbounded capacity — UIL-10/13) are the materially-exercised
#: governing conditions. C3's PL-F2/SF-2/AF-3/DF-2 hosting is scoped to the Distribution/
#: Storage-Hosting units; for Compute it is satisfied by the resource hosting the frozen
#: RL-F2 *execution* concern by reference, recorded with a scoping note.
_COMPLIANCE_CHECKS: dict[str, tuple[str, ...]] = {
    "C1": ("infra-compute-typed", "infra-compute-identified-objectbound"),
    "C2": ("foundation-reuse-integrity",),
    "C3": ("infra-compute-hosts-execution-by-reference", "hosting-by-reference"),
    "C4": ("infra-compute-declares-capacity-locality", "infra-compute-typed"),
    "C5": ("founding-acyclic", "meta-relationships-closed"),
    "C6": ("infra-compute-hosts-execution-by-reference", "scaling-unbounded"),
    "C7": ("non-constitutive", "technology-independence"),
}

_C3_NOTE = (
    "PL-F2/SF-2/AF-3 delivery + DF-2 data-hosting scoped to the Distribution/"
    "Storage-Hosting units (INFRASTRUCTURE-008/009/010); a Compute Resource hosts the "
    "frozen RL-F2 execution concern by reference (ICMP-01 / UIL-10)"
)


@dataclass(frozen=True, slots=True)
class ComputeComplianceReport:
...
def evaluate_compute_compliance(validation: ComputeValidation) -> ComputeComplianceReport:
...
@dataclass(frozen=True, slots=True)
class ComputeCertification:
    """The bundled outcome of certifying a validated Compute Resource."""

    decision: CertificationDecision
    evidence: CertificationEvidence
    ledger_entry: CertificationLedgerEntry
    ledger: CertificationLedger
    compliance: ComputeComplianceReport
...
def certify_compute(
    ...
    *,
    ...
    compliance = evaluate_compute_compliance(validation)
    return ComputeCertification(
        decision=decision,
