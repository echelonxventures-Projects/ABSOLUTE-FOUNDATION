"""EC3-B13-U01 — Realization orchestrator + evidence emitter.
...
Charter §3.2/§4.1 GATE 1): constructs the canonical Universal Infrastructure Capability
exemplar, validates it through the CERTIFIED EC-1 engine (meta-validity WF-1…12 +
Infrastructure-law UIL-01…15), certifies it through the CCE ten gates (CC-1…CC-10) +
Infrastructure compliance (C1…C7), closes its No-Orphan traceability, and emits a
**deterministic, content-addressed evidence bundle**. A double-realization self-check
proves byte-identical determinism (VC-4).

Run as a module::

    python -m infrastructure.capability_realize
    python -m infrastructure.capability_realize --evidence-dir infrastructure/_evidence/EC3-B13-U01
...
from infrastructure import REALIZATION_UNIT
...
from infrastructure.capability_traceability import TraceabilityRecord, build_traceability
from infrastructure.capability_validation import CapabilityValidation, validate_capability

#: The realized artifact version (version-pinned certification).
UNIT_VERSION = "1.0.0"

#: The canonical Universal Infrastructure Capability exemplar realized to prove the
#: construct: a Hosting capability that enables RL-F2 execution hosting, reusing the
#: PLATFORM-006 capability construct by reference.
CANONICAL_TYPE_TAG = "ucos.infrastructure.capability.foundation"
CANONICAL_ENABLES_REF = "ENG-005:RL-F2:runtime.execution"
...
#: applicable to the Capability). WF-4…WF-10 are scoped to other leaf meta-classes.
_META_VALIDITY_CHECKS: dict[str, str] = {
    "WF-1": "meta-class-single",
    "WF-2": "hosting-by-reference",
...
_UIL_CHECKS: dict[str, str] = {
    ...
    "UIL-03": "infra-capability-typed",
    "UIL-04": "infra-capability-identified-objectbound",
    "UIL-05": "infra-capability-identified-objectbound",
    "UIL-06": "hosting-by-reference",
    "UIL-10": "behavior-by-reference",
    "UIL-13": "scaling-unbounded",
    "UIL-15": "technology-independence",
}


def build_canonical_capability() -> InfrastructureCapability:
    """Construct the canonical Hosting Universal Infrastructure Capability exemplar."""
...
class RealizationResult:
    """The full, deterministic outcome of the EC3-B13-U01 realization act."""
    ...
    def uil_conformance(self) -> dict[str, bool]:
        ...
        result["UIL-01"] = True
        # UIL-14 (security/governance evaluative, non-enforcing) — the capability enacts
        # nothing and confers no authority.
        result["UIL-14"] = not self.capability.enacts_enforcement()
    ...
    def acceptance_criteria(self) -> dict[str, bool]:
        ...
        return {
            ...
            "AC-3": (
                passed.get("infra-capability-typed", False)
                and passed.get("infra-capability-identified-objectbound", False)
            ),
    ...
    def validation_criteria(self, *, byte_identical: bool) -> dict[str, bool]:
        ...
        return {
            ...
            "VC-3": all(self.uil_conformance().values()),  # UIL-01…15 conformance
    ...
    def capability_compliance(self) -> dict[str, bool]:
    ...
    def determination(self, *, byte_identical: bool) -> str:
        ...
        if (
            ...
            and all(vc.values())
            and gates_ok
            and all(self.capability_compliance().values())
        ):
            return "COMPLETE"
    ...
    def to_bundle(self, *, byte_identical: bool) -> dict[str, Any]:
        ...
        return {
            "artifact": "EC3-B13-U01-REALIZATION-EVIDENCE",
            ...
            "criteria": {
                ...
                "infrastructure_compliance": self.capability_compliance(),
            },
...
def realize() -> RealizationResult:
    ...
    trace = build_traceability(
        ...
        forward=(
            ...
            "EC3-B13-U01-VALIDATION-EVIDENCE",
            "EC3-B13-U01-CCE-CERTIFICATION",
            "EC3-B13-U01-COMPLETION-REPORT",
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    """CLI entry: realize EC3-B13-U01, emit evidence, print the determination."""
    parser = argparse.ArgumentParser(
        prog="ec3-b13-u01-realize",
        description="Realize the Universal Infrastructure Capability and emit evidence.",
    )
    ...
    print(f"[{'PASS' if complete else 'FAIL'}] EC3-B13-U01 → {summary['determination']}")
